#ifndef ASTRO_ACCELERATE_DEVICE_MSD_CONFIGURATION_HPP
#define ASTRO_ACCELERATE_DEVICE_MSD_CONFIGURATION_HPP
#include "params.hpp"
...
public:
  ...
  void reset() {
    ...
    nSteps.y             = 0;
    nSteps.y             = 0;
    partials_gridSize.x  = 1;
    partials_gridSize.y  = 1;
    partials_gridSize.z  = 1;
    partials_blockSize.x = 1;
    partials_blockSize.y = 1;
    partials_blockSize.z = 1;
    final_gridSize.x     = 1;
    final_gridSize.y     = 1;
  ...
  MSD_Configuration(void) { reset(); }

  MSD_Configuration(size_t t_nTimesamples,
                    size_t t_nDMs,
                    size_t t_offset,
                    int    t_address) {
  ...
  void print() {
    printf("         nTimesamples:%zu; nDMs:%zu; offset:%zu;\n",
           ...
           nDMs,
           offset);
    printf("         nBlocks:[%d;%d]; nSteps:[%d;%d]; nBlocks_total:%d; "
           "address:%d;\n",
           nBlocks.x,
           nBlocks.y,
           nSteps.x,
           ...
           nBlocks_total,
           address);
    printf("         partials_gridSize=[%d;%d;%d]; "
           "partials_blockSize=[%d;%d;%d]\n",
           partials_gridSize.x,
           partials_gridSize.y,
           partials_gridSize.z,
           partials_blockSize.x,
           partials_blockSize.y,
           partials_blockSize.z);
