...
__device__ __inline__ ushort is_peak(const float original_value,
                                     const float dilated_value,
                                     const float threshold) {
  return (original_value > threshold && original_value == dilated_value) ? 1u
                                                                         : 0u;
}
...
__device__ __inline__ float4 load_block_2x2(const float *input, int width) {
  ...
  second.x = __ldg(input + width);
  second.y = __ldg(input + width + 1);
  float4 result;
  result.x = first.x;
  result.y = first.y;
  result.z = second.x;
