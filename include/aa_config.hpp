...
public:
    ...
    const aa_compute::pipeline setup() {
      return setup(m_ddtr_plan, fpath);
    }
  
    //This specialisation of the setup method happens only if reading from an input_file
  const aa_compute::pipeline setup(aa_ddtr_plan &ddtr_plan, std::string &file_path) {
	  ...
	  if (( fp_in = fopen(user_cli.input[1].c_str(), "r") ) == NULL) {
	    ...
	    // temporary variables to read dm range
		  ...
		  m_ddtr_plan.add_dm(temp_low, temp_high, temp_step, temp_in_bin, temp_out_bin);
            ...
            rewind(fp_in);
	    pipeline.insert(aa_compute::modules::dedispersion);//Always add dedispersion to the pipeline
            while (!feof(fp_in)) {
	      if ( fscanf(fp_in, "%s", string) == 0) {
                    fprintf(stderr, "failed to read input\n");
                    return false;
		  ...
		  pipeline.insert(aa_compute::modules::analysis);
                if (strcmp(string, "periodicity") == 0)
		  pipeline.insert(aa_compute::modules::periodicity);
                if (strcmp(string, "acceleration") == 0)
		  pipeline.insert(aa_compute::modules::acceleration);
                if (strcmp(string, "output_ffdot_plan") == 0)
		  pipeline.insert(aa_compute::modules::output_ffdot_plan);
                if (strcmp(string, "output_fdas_list") == 0)
		  pipeline.insert(aa_compute::modules::output_fdas_list);
                if (strcmp(string, "output_dmt") == 0)
		  pipeline.insert(aa_compute::modules::output_dmt);
                if (strcmp(string, "zero_dm") == 0)
		  pipeline.insert(aa_compute::modules::zero_dm);
                if (strcmp(string, "zero_dm_with_outliers") == 0)
		  pipeline.insert(aa_compute::modules::zero_dm_with_outliers);
                if (strcmp(string, "rfi") == 0)
		  pipeline.insert(aa_compute::modules::rfi);
                if (strcmp(string, "oldrfi") == 0)
		  pipeline.insert(aa_compute::modules::old_rfi);
                if (strcmp(string, "threshold") == 0)
		  pipeline.insert(aa_compute::modules::candidate_algorithm);
                if (strcmp(string, "baselinenoise") == 0)
		  pipeline.insert(aa_compute::modules::sps_baseline_noise);
                if (strcmp(string, "fdas_custom_fft") == 0)
		  pipeline.insert(aa_compute::modules::fdas_custom_fft);
                if (strcmp(string, "fdas_inbin") == 0)
		  pipeline.insert(aa_compute::modules::fdas_inbin);
                if (strcmp(string, "fdas_norm") == 0)
		  pipeline.insert(aa_compute::modules::fdas_norm);
