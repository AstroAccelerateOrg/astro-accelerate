#ifndef ASTRO_ACCELERATE_DEVICE_MSD_SHARED_KERNEL_FUNCTIONS_CUH
#define ASTRO_ACCELERATE_DEVICE_MSD_SHARED_KERNEL_FUNCTIONS_CUH
      ...
      if(((int)jv) != 0) {
        if((*j) == 0) {
          ...
          Merge(M,
                S,
                j,
                s_input[i + threadIdx.x],
                s_input[blockDim.x + i + threadIdx.x],
                jv);
        }
      }
  ...
  for(int i = (blockDim.x >> 1); i > HALF_WARP; i = i >> 1) {
    if(threadIdx.x < i) {
      Merge(M,
            ...
            s_input[i + threadIdx.x],
            s_input[blockDim.x + i + threadIdx.x],
            s_input[2 * blockDim.x + i + threadIdx.x]);
