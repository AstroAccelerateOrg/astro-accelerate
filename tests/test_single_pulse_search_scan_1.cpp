...
int check_boxcar_results(float *h_input, float *h_GPU_boxcar_values, float *h_GPU_decimated, float *h_GPU_output_SNR, ushort *h_GPU_output_taps, int nBoxcars, size_t nTimesamples, size_t nDMs){
...
int main(void) {
	...
	for(int f=0; f<nBoxcars; f++) {
		...
		h_MSD_interpolated[f*2 + 1] = sqrt(f);
	}
	...
	int pass = check_boxcar_results(h_input, h_boxcar_values, h_decimated, h_output_SNR, h_output_taps, nBoxcars, (size_t) nTimesamples, (size_t) nDMs);
