...
1. Add any configuration flags for the module to `struct aa_config_flags`, and in the `get_user_input()` method implement that the `input_file` is checked if the user provided them in the `input_file`.
2. The module must now be added to `src/aa_main.cpp` which implements the standalone executable functionality. Create a plan object for the new module, passing the flags from the `aa_config` that read the user input. Surround the `bind` call by a simple check to see if the plan is required, for example `if(pipeline.find(aa_compute::modules::fdas) != pipeline.end())` - this makes sure the bind call only happens if the user actually requested a particular module.
