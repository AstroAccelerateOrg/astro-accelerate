...
/*
__global__ void MSD_BLN_pw_rejection(float const* __restrict__ d_input, float *d_output, int y_steps, int nTimesamples, int offset, float multiplier, float signal_mean, float signal_sd) {
	__shared__ float s_input[3*MSD_PW_NTHREADS];
	float M, S, j, ftemp;
	
	int spos = blockIdx.x*MSD_BLN_PW_EPT*MSD_PW_NTHREADS + threadIdx.x;
	int gpos = blockIdx.y*y_steps*nTimesamples + spos;
	M=0;	S=0;	j=0;
	if( spos<(nTimesamples-offset) ){
		...
		for (int yf = 0; yf < y_steps; yf++) {
			ftemp=__ldg(&d_input[gpos]);
			if( (ftemp > (signal_mean - multiplier*signal_sd)) && (ftemp < (signal_mean + multiplier*signal_sd)) ){
				if(j==0){
					Initiate( &M, &S, &j, ftemp);
				}
				else{
...
__global__ void MSD_GPU_LA_ALL_Nth_no_rejection(float const* __restrict__ d_input, float const* __restrict__ d_bv_in, float *d_output, float *d_output_taps, int y_steps, int nTaps, int nTimesamples, int offset) {
	...
	if (threadIdx.x == 0) {
		...
		d_output_taps[3*gpos + 1] = S;
		d_output_taps[3*gpos + 2] = j;
		if(S!=S) printf("M=%f; S=%f; j=%f\n", M, S, j);
		
		d_output[3*gpos] = M_b;
		d_output[3*gpos + 1] = S_b;
		d_output[3*gpos + 2] = j_b;
		if(S_b!=S_b) printf("M_b=%f; S_b=%f; j_b=%f\n", M_b, S_b, j_b);
	}
}
...
__global__ void MSD_GPU_LA_ALL_pw_rejection(float const* __restrict__ d_input, float *d_output, float *d_output_taps, float *d_MSD_T, float *d_MSD_T_base, float bln_sigma_constant, int y_steps, int nTaps, int nTimesamples, int offset) {
	...
	if (threadIdx.x == 0) {
		...
		d_output_taps[3*gpos + 1] = S;
		d_output_taps[3*gpos + 2] = j;
		if(S!=S) printf("pw_rejection final: M:%f; S:%f; j:%f\n", M, S, j);
		
		d_output[3*gpos] = M_b;
		d_output[3*gpos + 1] = S_b;
		d_output[3*gpos + 2] = j_b;
		if(S_b!=S_b) printf("pw_rejection final: M_b:%f; S_b:%f; j_b:%f\n", M_b, S_b, j_b);
	}
}
...
/*
__global__ void MSD_GPU_limited_final_create_linear_approx_regular(float *d_input, float *d_output, float *d_MSD_base, int nTaps, int size) {
	...
	if (threadIdx.x == 0) {
		...
		printf("MSD_GPU_limited_final_create_linear_approx_regular: mean:%f; sd:%f; j:%f; \n", M / j, sqrt(S / j), (sqrt(S / j) - d_MSD_base[1])/( (float) (nTaps-1)));
	}
}
*/

/*
__global__ void MSD_GPU_limited_final_create_linear_approx_nonregular(float *d_input, float *d_output, float *d_MSD_base, int nTaps, int size) {
	__shared__ float s_input[3*WARP*WARP];
	...
	if (threadIdx.x == 0) {
		...
		d_output[1] = sqrt(S / j);
		d_output[2] = (sqrt(S / j) - d_MSD_base[1])/( (float) (nTaps-1));
		printf("MSD_GPU_limited_final_create_linear_approx_nonregular: mean:%f; sd:%f; j:%f; \n", M / j, sqrt(S / j), (sqrt(S / j) - d_MSD_base[1])/( (float) (nTaps-1)));
	}
}
*/
...
/*
__global__ void MSD_GPU_limited_final_create_linear_approx_nonregular_final(float *d_input, float *d_output, float *d_MSD_base, int nTaps, int size) {
	__shared__ float s_input[3*WARP*WARP];
...
/*
__global__ void MSD_GPU_limited_final_create_linear_approx_nonregular_final_Nth(float *d_input, float *d_output, float *d_MSD_base, float *d_MSD_DIT, int nTaps, int size, int DIT_value) {
	__shared__ float s_input[3*WARP*WARP];
	...
	if (threadIdx.x == 0) {
		...
		d_output[2] = (sqrt(S / j) - d_MSD_base[1])/( (float) (nTaps));
		d_output[3] = d_MSD_DIT[0]*DIT_value;//*DIT_value
		printf("MSD_GPU_limited_final_create_linear_approx_nonregular: mean:%f; sd:%f; j:%f; DIT_mean:%f \n", M / j, sqrt(S / j), (sqrt(S / j) - d_MSD_base[1])/( (float) (nTaps)), d_MSD_DIT[0]*DIT_value);
	}
}
*/
#endif
...
__global__ void MSD_GPU_LA_ALL_pw_rejection(float const* __restrict__ d_input, float *d_output, float *d_output_taps, float *d_MSD_T, float *d_MSD_T_base, float bln_sigma_constant, int y_steps, int nTaps, int nTimesamples, int offset) {
	...
	if( (spos+4)<(nTimesamples-offset) ){
		...
		if( (ftemp1.x > (signal_mean - bln_sigma_constant*signal_sd)) && (ftemp1.x < (signal_mean + bln_sigma_constant*signal_sd)) ){
			...
			//if(S_b!=S_b) printf("first th:%d; M_b:%f; S_b:%f; j_b:%f [%f;%f]\n", threadIdx.x, M_b, S_b, j_b, ftemp1.x, 0);
		}
		if( (ftemp1.y > (signal_mean - bln_sigma_constant*signal_sd)) && (ftemp1.y < (signal_mean + bln_sigma_constant*signal_sd)) ){
			if(j_b==0) Initiate( &M_b, &S_b, &j_b, ftemp1.y );
			else Add_one( &M_b, &S_b, &j_b, ftemp1.y );
			//if(S_b!=S_b) printf("first th:%d; M_b:%f; S_b:%f; j_b:%f [%f;%f]\n", threadIdx.x, M_b, S_b, j_b, 0, ftemp1.y);
		}
	...
	spos=blockIdx.x*(EpT) + 2*threadIdx.x;	
	// correct results have threads with 0 <= local_id < (2*MSD_PW_NTHREADS-nTaps); All other threads needs to store j=0  
	if(local_id>=0) {		
		// Note: ommited number of samples in the last trailing threadblocks is due to -nTaps which is here. 
		//       Missing data should be contained in local_id. Thus this code is missing some time sample even it it does not need to. 
		//       When removed it produces different number of added time samples in j and j_b which is wierd
		if( spos<(nTimesamples-offset-nTaps) ) { // -nTaps might not be necessary
			...
			s_base[blockDim.x + local_id] = S_b;
			s_base[2*blockDim.x + local_id] = j_b;
			if(S_b!=S_b) printf("th:%d; M_b:%f; S_b:%f; j_b:%f\n", threadIdx.x, M_b, S_b, j_b);
		}
