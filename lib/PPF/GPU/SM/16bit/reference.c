...
void FIR_check(short2 *input_data, float2 *spectra_GPU, float *coeff, int nTaps, int nChannels, int nSpectra, double *cumulative_error, double *mean_error){
	float2 ftemp;
	double etemp=0;
	double error=0;
	int count=0;
	for(int bl=0;bl<nSpectra;bl++){
		for(int c=0;c<nChannels;c++){
			...
			for(int t=0;t<nTaps;t++){
				ftemp.x+=coeff[t*nChannels + c]*(float)input_data[bl*nChannels + t*nChannels + c].x;
				ftemp.y+=coeff[t*nChannels + c]*(float)input_data[bl*nChannels + t*nChannels + c].y;
			}//nTaps
...
void FIR_FFT_check(short2 *input_data, float2 *spectra_GPU, float *coeff, int nTaps, int nChannels, int nSpectra, double *cumulative_error, double *mean_error){
	float2 *spectra;
	...
	for(int bl=0;bl<nSpectra;bl++){
		for(int c=0;c<nChannels;c++){
			...
			for(int t=0;t<nTaps;t++){
				spectra[c].x+=coeff[t*nChannels + c]*(float)input_data[bl*nChannels + t*nChannels + c].x;
				spectra[c].y+=coeff[t*nChannels + c]*(float)input_data[bl*nChannels + t*nChannels + c].y;
			}//nTaps
		}//nChannels
