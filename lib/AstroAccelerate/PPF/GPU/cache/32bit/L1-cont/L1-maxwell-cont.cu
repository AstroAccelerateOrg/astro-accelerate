...
// ------------> maxwell and kepler
__global__ void Fir_L1(float2 const* __restrict__ d_data, float2* d_spectra, float const* __restrict__ d_coeff, int nTaps, int nChannels, int yshift) {
	int t = 0;
	...
	for(t=threadIdx.x + yshift; t<(nTaps)*nChannels; t+=nChannels){
	  ...
	  for(int i=0;i<SPECTRA_PER_BLOCK;i++){
	    ...
	    ftemp[i].x += temp*utemp.x;
	    ftemp[i].y += temp*utemp.y;
	  }
	}
...
void GPU_Polyphase(float2 *input, float2 *output, float *coeff, int nChannels, int nTaps, int nSpectra){
	...
	timer.Start();
	if(nRepeats>0){
		checkCudaErrors(cudaMemcpy(d_input, input, input_size*sizeof(float2), cudaMemcpyHostToDevice));
	}
	else {
		checkCudaErrors(cudaMemcpy(d_input, &input[output_size*nRepeats], (Sremainder+nTaps-1)*nChannels*sizeof(float2), cudaMemcpyHostToDevice));
	}
	timer.Stop();
	...
	for (int r = 0; r < nRepeats; r++){
		...
		for (int nutak=0;nutak<nKernels;nutak++){	
			Fir_L1<<<GridSize, BlockSize>>>((float2*) d_input, (float2*) d_output, d_coeff, nTaps, nChannels, nutak*THREADS_PER_BLOCK);
		}
		if (Kremainder>0){
			BlockSize.x=Kremainder;
			Fir_L1<<<GridSize, BlockSize>>>((float2*) d_input, (float2*) d_output, d_coeff, nTaps, nChannels, nKernels*THREADS_PER_BLOCK);
		}
		timer.Stop();
