...
void GPU_Polyphase(uchar4 *input, float2 *output, float *coeff, int nChannels, int nTaps, int nSpectra){
	...
	cudaMemGetInfo(&free_mem,&total_mem);
	printf("\nDevice has %f MB of total memory, which %f MB is available.\n", (double) total_mem/(1000.0*1000.0), (double) free_mem/(1000.0*1000.0));
	...
	if (DEBUG) printf("Maximum number of spectra %d which is %e MB \n",nColumns, (double) (nColumns*nChannels*sizeof(float2)/(1000.0*1000.0))   );
