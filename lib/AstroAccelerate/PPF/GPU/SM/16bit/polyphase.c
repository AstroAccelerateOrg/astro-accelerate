...
int main(int argc, char* argv[]) {
	...
	if (DEBUG) printf("\nHost memory allocation...\t");
		h_input 	= (short2 *)malloc(input_size*sizeof(short2));
	...
	if (DEBUG && CHECK){ 
		printf("\nTesting FIR...\t\t");
		FIR_check((short2*) h_input, h_output, h_coeff, nTaps, nChannels, nSpectra, &cumulative_error, &mean_error);
		printf("Cumulative Error: %e, Mean Error: %e;",cumulative_error,mean_error);
	}
	*/
	if (DEBUG && CHECK){
		printf("\nTesting FIR+FFT...\t");
		FIR_FFT_check((short2*) h_input, h_output, h_coeff, nTaps, nChannels, nSpectra, &cumulative_error, &mean_error);
		printf("Cumulative Error: %e, Mean Error: %e;",cumulative_error,mean_error);
	}
