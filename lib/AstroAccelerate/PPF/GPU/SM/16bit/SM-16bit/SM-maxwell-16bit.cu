...
// Maxwell and Kepler
__global__ void Fir_shared_16bit(short2 const* __restrict__ d_data, float2* d_spectra, float const* __restrict__ d_coeff, int nChannels) {
	
	float2 ftemp;
	int memblock, localId, s_mempos, g_mempos, num_spectra, start_column, warpId, itemp;
	int tx = threadIdx.x;
	
	__shared__ short2 s_data[DATA_SIZE];
	...
	for (int i = 0; i < SUBBLOCK_SIZE; i++){
		...
		if (start_column<DATA_SIZE/WARP) {
			...
			s_data[s_mempos] = __ldg(&d_data[g_mempos]);
		}
	}
	...
	for (int i = 0; i < SUBBLOCK_SIZE; i++){
		...
		if (start_column < num_spectra){
			...
			for (int j = 0; j < TAPS; j++){
				ftemp.x += s_coeff[j*WARP + localId]*((float) s_data[s_mempos + j*WARP].x);
				ftemp.y += s_coeff[j*WARP + localId]*((float) s_data[s_mempos + j*WARP].y);
			} //ntaps
...
__global__ void Fir_shared_16bit(short2 const* __restrict__ d_data, float2* d_spectra, float const* __restrict__ d_coeff, int nChannels) {
	...
	for (int i = 0; i < SUBBLOCK_SIZE; i++){
		start_column = memblock+i;
		if (start_column < num_spectra){
			s_mempos = memblock*WARP + WARP*i+localId;
			ftemp.x = 0.0f;ftemp.y = 0.0f;
			for (int j = 0; j < TAPS; j++){
				ftemp.x += s_coeff[j*WARP + localId]*((float) s_data[s_mempos + j*WARP].x);
				ftemp.y += s_coeff[j*WARP + localId]*((float) s_data[s_mempos + j*WARP].y);
...
int Max_columns_in_memory(int nTaps, int nChannels) {
	...
	maxColumns=((long int) free_mem - nChannels*sizeof(float)*TAPS -(TAPS-1)*nChannels*sizeof(float2))/(2.0*sizeof(float2)*nChannels + sizeof(short2)*nChannels);
...
void Polyphase_GPU_benchmark(short2 *d_input, float2 *d_output, float *d_coeff, int nChannels, int nTaps, int nSpectra, double *fir_time, double *fft_time){
	GpuTimer timer;
	...
	//---------> FIR filter part
	timer.Start();
	Fir_shared_16bit<<<gridSize, blockSize>>>( d_input, d_output, d_coeff, nChannels);
	timer.Stop();
...
void GPU_Polyphase(short2 *input, float2 *output, float *coeff, int nChannels, int nTaps, int nSpectra){
	...
	for (int r = 0; r < nRepeats; r++){
		...
		timer.Start();
		checkCudaErrors(cudaMemcpy(d_input, &input[r*output_size], input_size*sizeof(short2), cudaMemcpyHostToDevice));
		timer.Stop();
		transfer_in+=timer.Elapsed();
	...
	if (Sremainder>0){
		//-----> Copy chunk of input data to a device
		timer.Start();
		checkCudaErrors(cudaMemcpy(d_input, &input[nRepeats*output_size], (Sremainder+TAPS-1)*nChannels*sizeof(short2), cudaMemcpyHostToDevice));
		timer.Stop();
		transfer_in+=timer.Elapsed();
