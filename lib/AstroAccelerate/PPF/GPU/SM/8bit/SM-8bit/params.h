...
// DATA_SIZE says how many input data elements (in floating point numbers) we want to store in shared memory per thread-block. DATA_SIZE=TOT_SM_SIZE/ACTIVE_BLOCKS=4096; DATA_SIZE=DATA_SIZE-COEFF_SIZE=3584; this is because we need to store coefficients in the shared memory along with the input data. We do not need to divide it by two as it was in case of 32-bit precision because we store both real and imaginary part into one bank via uchar4 data type. 
...
//Note: in case of 8-bit implementations the things are more complicated. For Fermi generations we have used different scheme. We are loading full uchar4 (in Maxwell and Kepler we load uchar2 and store it into uchar4) this means that each thread processes two channels and not only one. This also means we need more coefficients, thus when calculating COEFF_SIZE we need to multiply it by two. Thus in this case:
// COEFF_SIZE is given by number of taps and channels processed per thread-block COEFF_SIZE=CHANNELS_PER_BLOCK*TAPS*2=1024
//#define COEFF_SIZE 512
// DATA_SIZE says how many input data elements (in floating point numbers) we want to store in shared memory per thread-block. DATA_SIZE=TOT_SM_SIZE/ACTIVE_BLOCKS=4096; DATA_SIZE=DATA_SIZE-COEFF_SIZE=3072; this is because we need to store coefficients in the shared memory along with the input data. We do not need to divide it by two as it was in case of 32-bit precision because we store both real and imaginary part into one bank via uchar4 data type. 
//#define DATA_SIZE 3072
// THREADS_PER_BLOCK gives number of threads per thread-block. It could be calculated as such THREADS_PER_BLOCK=MAX_THREADS_PER_SM/ACTIVE_BLOCKS; rounded to nearest lower multiple of 32; In case of Maxwell generation MAX_THREADS_PER_SM=2048, thus THREADS_PER_BLOCK=682.6666, rounding to nearest lower multiple of 32 gives THREADS_PER_BLOCK=672;
//#define THREADS_PER_BLOCK 672
// SUBBLOCK_SIZE gives size of the sub-block as given in our article. It is calculated as ratio of DATA_SIZE and THREADS_PER_BLOCK rounded up so SUBBLOCK_SIZE=DATA_SIZE/THREADS_PER_BLOCK=4.57, rounding up gives SUBBLOCK_SIZE=5;
//#define SUBBLOCK_SIZE 5
