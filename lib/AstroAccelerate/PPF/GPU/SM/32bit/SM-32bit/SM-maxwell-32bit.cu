...
// Maxwell
__global__ void Fir_shared_32bit(float2 const* __restrict__ d_data, float2* d_spectra, float const* __restrict__ d_coeff, int nChannels) {
	...
	localId= tx - ((int) tx/WARP)*WARP;		// Calculates threads Id within a WARP
	...
	for (int i = 0; i < SUBBLOCK_SIZE; i++){
		...
		if (start_column<DATA_SIZE/WARP) {
			...
			ftemp=__ldg(&d_data[g_mempos]);
			s_data[s_mempos] = ftemp.x;
			s_data[s_mempos+DATA_SIZE] = ftemp.y; // storing data in {{real, real, real, ...},{imag, imag, ...}} allows us to prevent bank conflicts
		}
	}
	
	// read coefficients from global memory and store them into shared memory
	...
	for (int i = 0; i < SUBBLOCK_SIZE; i++){ // WARP loops through columns in it's sub-block
		...
		if (start_column < num_spectra){
			...
			for (int j = 0; j < TAPS; j++){
				ftemp.x += s_coeff[j*WARP + localId]*(s_data[s_mempos + j*WARP]);
				ftemp.y += s_coeff[j*WARP + localId]*(s_data[s_mempos + j*WARP + DATA_SIZE]);
			} //ntaps
			d_spectra[localId + start_column*nChannels + blockIdx.x*WARP + blockIdx.y*num_spectra*nChannels] = ftemp;
		} //if
	} // number of columns
	//return;
}
...
// Kepler
/*
__global__ void Fir_shared(float2 const* __restrict__ d_data, float2* d_spectra, float const* __restrict__ d_coeff, int nChannels) {
	
	float2 ftemp;
	int memblock, localId, mempos, num_spectra, start_column, x, warpId;
	int tx = threadIdx.x;
	
	__shared__ float2 s_data[DATA_SIZE];
	__shared__ float2 s_coeff[COEFF_SIZE];
	...
	for (int i = 0; i < SUBBLOCK_SIZE; i++){
		start_column = memblock+i;
		if (start_column<DATA_SIZE/WARP) {
			mempos = start_column*WARP + localId;
			x = localId + start_column*nChannels + blockIdx.x*WARP + blockIdx.y*num_spectra*nChannels;
			s_data[mempos] = __ldg(&d_data[x]);
		}
	}
	
	x=(int) (TAPS/(THREADS_PER_BLOCK/WARP)) + 1;
	for(int f=0;f<x;f++){
		start_column=warpId+f*(THREADS_PER_BLOCK/WARP);
		if(start_column<TAPS){
			s_coeff[start_column*WARP+localId].x=__ldg(&d_coeff[start_column*nChannels + blockIdx.x*WARP + localId]);
		}
	}
	...
	for (int i = 0; i < SUBBLOCK_SIZE; i++){
		...
		if (start_column < num_spectra){
			...
			ftemp.x = 0.0f;ftemp.y = 0.0f;
			for (int j = 0; j < TAPS; j++){
				ftemp.x += s_coeff[j*WARP + localId].x*(s_data[mempos + j*WARP].x);
				ftemp.y += s_coeff[j*WARP + localId].x*(s_data[mempos + j*WARP].y);
			} //ntaps
			d_spectra[localId + start_column*nChannels + blockIdx.x*WARP + blockIdx.y*num_spectra*nChannels] = ftemp;
		} //if
	} // number of columns
...
__global__ void Fir_shared(float2 const* __restrict__ d_data, float2* d_spectra, float const* __restrict__ d_coeff, int nChannels) {
	...
	for (int i = 0; i < SUBBLOCK_SIZE; i++){
		...
		if (start_column<DATA_SIZE/WARP) {
			s_mempos = start_column*WARP + localId;
			g_mempos = localId + start_column*nChannels + blockIdx.x*WARP + blockIdx.y*num_spectra*nChannels;
			s_data[s_mempos] = d_data[g_mempos].x;
			s_data[s_mempos+DATA_SIZE] = d_data[g_mempos].y;
		}
	}
	...
	for(int f=0;f<itemp;f++){
		start_column=warpId+f*(THREADS_PER_BLOCK/WARP);
		if(start_column<TAPS){
			s_coeff[start_column*WARP+localId]=d_coeff[start_column*nChannels + blockIdx.x*WARP + localId];
		}
	}
...
//This gives maximum number of columns with nChannels which can be stored in memory
int Max_columns_in_memory(int nTaps, int nChannels) {
	...
	maxColumns=((long int) free_mem - nChannels*sizeof(float)*TAPS -(TAPS-1)*nChannels*sizeof(float2))/(3.0*sizeof(float2)*nChannels);
...
void Polyphase_GPU_benchmark(float2 *d_input, float2 *d_output, float *d_coeff, int nChannels, int nTaps, int nSpectra, double *fir_time, double *fft_time){
	GpuTimer timer;
	...
	//---------> FIR filter part
	timer.Start();
	Fir_shared_32bit<<<gridSize, blockSize>>>( d_input, d_output, d_coeff, nChannels);
	timer.Stop();
...
void GPU_Polyphase(float2 *input, float2 *output, float *coeff, int nChannels, int nTaps, int nSpectra){
	...
	for (int r = 0; r < nRepeats; r++){
		...
		timer.Start();
		checkCudaErrors(cudaMemcpy(d_input, &input[r*output_size], input_size*sizeof(float2), cudaMemcpyHostToDevice));
		timer.Stop();
		transfer_in+=timer.Elapsed();
	...
	if (Sremainder>0){
		//-----> Copy chunk of input data to a device
		timer.Start();
		checkCudaErrors(cudaMemcpy(d_input, &input[nRepeats*output_size], (Sremainder+TAPS-1)*nChannels*sizeof(float2), cudaMemcpyHostToDevice));
		timer.Stop();
		transfer_in+=timer.Elapsed();
