...
// WA:SKA Modified so that the code processes data in 13 second chunks (meaning that approximatly 10 seconds of data will be de-dispersed and searched for single pulses).
		...
		// mb2 max_tsamps = (unsigned int) ( (3120140625) / ( sizeof(unsigned short)*nchans + sizeof(float)*(*max_ndms) + (size_t)(SPS_mem_requirement*MIN_DMS_PER_SPS_RUN ))); 
...
//		max_tsamps = (unsigned int) ( ( *gpu_memory ) / ( nchans * ( sizeof(float) + sizeof(unsigned short) )+ SPS_mem_requirement*MIN_DMS_PER_SPS_RUN ));
		...
		printf("\n%d", SPS_mem_requirement*MIN_DMS_PER_SPS_RUN );

// WA:SKA Modified so that the code processes data in 13 second chunks (meaning that approximatly 10 seconds of data will be de-dispersed and searched for single pulses).
                max_tsamps = (unsigned int) ( (2.5*3120140625) / ( sizeof(unsigned short)*nchans + sizeof(float)*(*max_ndms) + (size_t)(SPS_mem_requirement*MIN_DMS_PER_SPS_RUN )));           
