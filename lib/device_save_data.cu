...
void save_data_offset(float *host_pointer_pinned, int device_offset, float *host_pointer, int host_offset, size_t size, cudaStream_t streams)
{
	...
	//{{{ Copy data and set up the GPU constants/variables.
	memcpy(host_pointer_pinned + device_offset, host_pointer + host_offset,size);
//	cudaMemcpyAsync(host_pointer_small + host_offset, device_pointer + device_offset, size, cudaMemcpyDeviceToHost, streams);
