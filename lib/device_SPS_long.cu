...
// TODO: this also needs modified kernel since number of tams must be kept local (taps) and not global (star_taps+taps)
int PD_SEARCH_LONG_LINAPPROX_EACH(float *d_input, float *d_boxcar_values, float *d_decimated, float *d_output_SNR, ushort *d_output_taps, std::vector<PulseDetection_plan> *PD_plan, int max_iteration, int nDMs, int nTimesamples, double *MSD_time, double *SPDT_time) {
	GpuTimer timer;
	...
	timer.Start();
	if(nBlocks>0) PD_GPU_1st_LA<<<gridSize,blockSize>>>( d_input, d_boxcar_values, d_decimated, d_output_SNR, d_output_taps, d_MSD, decimated_timesamples, nBoxcars, dtm);
	timer.Stop(); *SPDT_time += timer.Elapsed();
...
int PD_SEARCH_LONG_BLN_LINAPPROX_EACH(float *d_input, float *d_boxcar_values, float *d_decimated, float *d_output_SNR, ushort *d_output_taps, std::vector<PulseDetection_plan> *PD_plan, int max_iteration, int nDMs, int nTimesamples, float sigma_constant, double *MSD_time, double *SPDT_time) {
	GpuTimer timer;
