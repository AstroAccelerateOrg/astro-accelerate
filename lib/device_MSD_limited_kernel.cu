...
__global__ void MSD_GPU_LA_ALL_func(float const* __restrict__ d_input, float *d_output, float *d_output_taps, int y_steps, int nTaps, int nTimesamples, int offset) {
	...
	// Initialization of MSD variables for processed StrDev
	Initiate( &M, &S, &j, Bw[0] );
	// First addition (second actually, but first done this way) processed StrDev
	Add_one( &M, &S, &j, Bw[1] );
...
__global__ void MSD_GPU_limited_final_func(float *d_input, float *d_output, int size) {
...
__global__ void MSD_GPU_limited_final_create_linear_approx_func(float *d_input, float *d_output, float *d_MSD_base, int nTaps, int size) {
	__shared__ float s_input[3*WARP*WARP];
