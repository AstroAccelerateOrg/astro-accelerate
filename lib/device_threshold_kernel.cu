...
__global__ void THR_GPU_WARP(float2 const* __restrict__ d_input, unsigned char *d_input_taps, float *d_output_list, int *gmem_pos, float threshold, int nTimesamples, int max_list_size) {
	int local_id;
	local_id = threadIdx.x & (WARP - 1);
	int warp_id;
	warp_id = threadIdx.x>>5;
	...
	for(int i=0; i<THR_ELEM_PER_THREAD; i++){
		...
		if(R.x > threshold) {
			...
			if(list_pos<max_list_size){
				...
				d_output_list[4*list_pos+1] = 2*((blockIdx.x*THR_ELEM_PER_THREAD + i)*WARP + local_id);
				d_output_list[4*list_pos+2] = R.x;
				d_output_list[4*list_pos+3] = d_input_taps[2*pos];
			}
		}
		if(R.y > threshold) {
			mask=__ballot(1);
			...
			if(list_pos<max_list_size){
				...
				d_output_list[4*list_pos+1] = 2*((blockIdx.x*THR_ELEM_PER_THREAD + i)*WARP + local_id)+1;
				d_output_list[4*list_pos+2] = R.y;
				d_output_list[4*list_pos+3] = d_input_taps[2*pos+1];
			}
		}
		pos = pos + WARP;
	}
