...
__global__ void PD_GPU_1st_float1_BLN_IF_ALIASING(float const* __restrict__ d_input, float *d_bv_out, float *d_decimated, float *d_output_SNR, ushort *d_output_taps, float *d_MSD, int nTimesamples, int nBoxcars) {
	__shared__ float2 s_input[PD_NTHREADS];
	__shared__ float2 s_SNRs[PD_NTHREADS];
	__shared__ ushort2 s_taps[PD_NTHREADS];
...
__global__ void PD_GPU_Nth_float1_BLN_IF_ALIASING(float const* __restrict__ d_input, float *d_bv_in, float *d_bv_out, float *d_decimated, float *d_output_SNR, ushort *d_output_taps, float *d_MSD, int nTimesamples, int nBoxcars, int startTaps, int DIT_value, int shift) {
	__shared__ float2 s_input[PD_NTHREADS];
	__shared__ float2 s_BV[PD_NTHREADS];
	__shared__ float2 s_SNRs[PD_NTHREADS];
	__shared__ ushort2 s_taps[PD_NTHREADS];
	...
	for(d=4; d<nBoxcars; d=d+4){
		...
		if(spos>=0){
			...
			if(ftemp1.x>SNR.x) {
				if(d==(nBoxcars-4)) {SNR.x = 0; taps.x = taps1.x;}
				else {SNR.x = ftemp1.x; taps.x = taps1.x;}
			}
			if(ftemp1.y>SNR.y) {
				if(d==(nBoxcars-4)) {SNR.y = 0; taps.y = taps1.y;}
				else {SNR.y = ftemp1.y; taps.y = taps1.y;}
			}
