...
__global__ void PD_GPU_Nth_float1_BLN_IF_OE(float const* __restrict__ d_input, float *d_bv_in, float *d_bv_out, float *d_decimated, float *d_output_SNR, ushort *d_output_taps, float *d_MSD, const int nTimesamples, const int nBoxcars, const int startTaps, const int DIT_value, const int dtm) {
	__shared__ float2 s_input[PD_NTHREADS];
...
__global__ void PD_GPU_Nth_float1_BLN_IF(float const* __restrict__ d_input, float *d_bv_in, float *d_bv_out, float *d_decimated, float *d_output_SNR, ushort *d_output_taps, float *d_MSD, int nTimesamples, int nBoxcars, const int startTaps, const int DIT_value, const int shift) {
	__shared__ float2 s_input[PD_NTHREADS];
	__shared__ float2 s_BV[PD_NTHREADS];
	__shared__ float2 s_SNRs[PD_NTHREADS];
	__shared__ ushort2 s_taps[PD_NTHREADS];
	...
	if( (spos+4)<nTimesamples ){
		...
		if( threadIdx.x<(PD_NTHREADS-(nBoxcars>>1)) ) d_decimated[(gpos>>1)]=Bw[1];
	}
