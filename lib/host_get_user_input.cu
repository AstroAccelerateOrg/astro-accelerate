	...
	else if (argc == 2 && strcmp(argv[1], "-help") != 0)
	{
		...
		*inBin = (int *) malloc(( *range ) * sizeof(int));
		...
		while (!feof(fp_in))
		{
			...
			if (strcmp(string, "max_boxcar_width") == 0)
				fscanf(fp_in, "%d", max_boxcar_width);
