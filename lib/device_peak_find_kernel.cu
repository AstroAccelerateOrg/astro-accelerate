...
__global__ void dilate_peak_find(const float *d_input, ushort* d_input_taps, float4 *d_peak_list, const int width, const int height, const int offset, const float threshold, int max_peak_size, int *gmem_pos, int shift, int DIT_value) {
	...
	if(peak==1){
		...
		if(list_pos<max_peak_size){
			d_peak_list[list_pos] = make_float4(float(idxY + shift),
                                                            float(idxX*DIT_value),
                                                            my_value, float(d_input_taps[idxY*width+idxX]));
		}
	}
