	...
	for(int i=0; i<(int) chunk_size.size(); i++){
		...
		for(size_t d=0; d<chunk_size[i]; d++) {
			for(size_t t=0; t<primary_dimension; t++) {
				h_export_data[d*primary_dimension + t] = h_data[(inner_sd_shift + d)*primary_dimension + t];
			}
		}
		...
		fwrite(h_data, primary_dimension*chunk_size[i]*sizeof(float), 1, fp_out);
		fclose(fp_out);
		
		inner_sd_shift = inner_sd_shift + chunk_size[i];
		printf("Exported: %0.2f%\n", ((float) inner_sd_shift)*percent_per_DMtrial);
	}
