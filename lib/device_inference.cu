...
*/
	...
	if (BSTRAP==1) bootstrap<<<num_blocks_boot,threads_per_block_boot>>>(num_bins, num_els, num_boots, d_bin_array,d_odata,d_irand);
	if (BSTRAP==2) bootstrap2<<<num_blocks_boot,threads_per_block_boot>>>(num_bins, num_els, num_boots, d_bin_array,d_odata,d_irand);
	if (BSTRAP==3) bootstrap3<<<num_blocks_boot,threads_per_block_boot>>>(num_bins, num_els, num_boots, d_bin_array,d_odata,d_irand);
        //cudaCheckMsg("bootstrap kernel execution failed");
	
	double *boots_array;
        boots_array = (double*) malloc(local_ndms*(num_boots)*sizeof(double));
        for (int i = 0 ; i<num_boots*local_ndms; i++) boots_array[i]=0.0;
