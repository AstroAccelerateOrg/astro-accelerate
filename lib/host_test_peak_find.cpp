...
TEST_CASE("Peak Find v4", "[peakfind]") {
  std::vector<float, managed_allocator<float>> data;
  std::vector<unsigned short, managed_allocator<unsigned short>> output;

  SECTION ("Empty data is ok") {
    peakfind_v4(data.data(), 0, 0, 0, output.data());
  }
  ...
  SECTION ("Single element is peak") {
    ...
    output.resize(1);
    data[0] = 1.0;
    peakfind_v4(data.data(), 0, 1, 1, output.data());

    REQUIRE(output[0] == 1);
  }

  SECTION ("Peak in top left") {
    test_peak_at_loc(data, output, 0, peakfind_v4);
  }

  SECTION ("Peak on top edge") {
    test_peak_at_loc(data, output, 1, peakfind_v4);
  }
  SECTION ("Peak in top right") {
    test_peak_at_loc(data, output, 2, peakfind_v4);
  }
  SECTION ("Peak on left edge") {
    test_peak_at_loc(data, output, 3, peakfind_v4);
  }
  SECTION ("Peak in centre") {
    test_peak_at_loc(data, output, 4, peakfind_v4);
  }
  SECTION ("Peak on right edge") {
    test_peak_at_loc(data, output, 5, peakfind_v4);
  }
  SECTION ("Peak in bottom left") {
    test_peak_at_loc(data, output, 6, peakfind_v4);
  }
  SECTION ("Peak on bottom edge") {
    test_peak_at_loc(data, output, 7, peakfind_v4);
  }
  SECTION ("Peak in bottom right") {
    test_peak_at_loc(data, output, 8, peakfind_v4);
  }
  ...
  SECTION ("Peak in neighbouring block") {
    ...
    SECTION("Right BC") {
        data[1024] = 10000.0f;
        data[1025] = 1.0f;
        peakfind_v4(data.data(), 0, 1024*2, 3, output.data());
        for(std::size_t i=0; i < data.size(); ++i) {
            auto val = (i==1024) ? 1u: 0u;
            REQUIRE(output[i]==val);
        }
  ...
  SECTION ("Peak across block corners") {
     ...
     SECTION("Top left block BR corner") {
         data[64*32-32-1] = 100.0f;
         data[64*32+32] = 1.0f;
         peakfind_v4(data.data(), 0, 64, 64, output.data());
         //debug_print_output(data, 64, 64);
         //debug_print_output(output, 64, 64);
         for(std::size_t i=0; i < data.size(); ++i) {
            auto val = (i==64*32-32-1) ? 1u: 0u;
            REQUIRE(output[i]==val);
         }
