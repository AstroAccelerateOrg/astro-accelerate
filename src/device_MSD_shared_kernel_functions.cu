...
__global__ void
MSD_GPU_final_regular(float* d_input, float* d_output, int size) {
...
__global__ void
MSD_GPU_final_regular(float* d_input, float* d_MSD, float* d_pp, int size) {
  __shared__ float s_input[3 * WARP * WARP];
...
__global__ void
MSD_GPU_final_nonregular(float* d_input, float* d_MSD, int size) {
  __shared__ float s_input[3 * WARP * WARP];
...
__global__ void
MSD_GPU_final_nonregular(float* d_input, float* d_MSD, float* d_pp, int size) {
  __shared__ float s_input[3 * WARP * WARP];
...
__global__ void MSD_GPU_Interpolate_linear(float* d_MSD_DIT,
  ...
  if(boxcar[tid] <= max_width_performed) {
    ...
    //      printf("\nBoxcar: %f \t desired: %f", (float)boxcar[f],
    //      desired_width);
    ...
    if(position == MSD_DIT_size - 1 && width1 == (int)desired_width) {
      ...
      //                        f); printf("width:[%f;%f]; mean:[%f;%f];
      //                        sd:[%f;%f]\n",width1, width2, mean1, mean2,
      //                        StDev1, StDev2); printf("d width %f; d mean: %f;
      //                        d StDef: %f\n", distance_in_width,
      //                        distance_in_mean, distance_in_StDev);
      //                        printf("\tDesired_width: %f\n", desired_width);

      //                (*mean) = mean1 +
      //                (distance_in_mean/distance_in_width)*((float)
      //                desired_width - width1);
      //                (*StDev) = StDev1 +
      //                (distance_in_StDev/distance_in_width)*((float)
      //                desired_width - width1);
      d_MSD_interpolated[tid * 2] =
          mean1 + (distance_in_mean / distance_in_width) *
                      ((float)desired_width - width1);
      d_MSD_interpolated[tid * 2 + 1] =
          StDev1 + (distance_in_StDev / distance_in_width) *
                       ((float)desired_width - width1);
    }
  }
}

void call_kernel_MSD_GPU_final_regular(const dim3&  grid_size,
...
void call_kernel_MSD_GPU_final_nonregular(const dim3&  grid_size,
                                          const dim3&  block_size,
                                          float* const d_input,
                                          float* const d_MSD,
                                          const int&   size) {
...
void call_kernel_MSD_GPU_final_nonregular(const dim3&  grid_size,
                                          ...
                                          float* const d_pp,
                                          const int&   size) {
  MSD_GPU_final_nonregular<<<grid_size, block_size>>>(
      d_input, d_MSD, d_pp, size);
}

void call_kernel_MSD_GPU_Interpolate_linear(const dim3&  grid_size,
                                            const dim3&  block_size,
                                            float* const d_MSD_DIT,
                                            float* const d_MSD_interpolated,
                                            int* const   d_MSD_DIT_widths,
                                            const int&   MSD_DIT_size,
                                            int* const   boxcar,
                                            const int&   max_width_performed) {
  MSD_GPU_Interpolate_linear<<<grid_size, block_size>>>(d_MSD_DIT,
                                                        d_MSD_interpolated,
                                                        d_MSD_DIT_widths,
                                                        MSD_DIT_size,
                                                        boxcar,
