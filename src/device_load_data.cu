...
// extern "C" void load_data(int i, float *device_pointer, float *host_pointer,
// size_t size, int nsamp, int maxshift, int nchans, int t_processed_s, int
// t_processed_c, float *dmshifts);
...
void load_data(int             i,
  ...
  cudaMemcpyToSymbol(
      c_sqrt_taps, h_sqrt_taps, (PD_MAXTAPS + 1) * sizeof(float));
}
