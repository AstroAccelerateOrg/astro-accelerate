...
void analysis_CPU(int    i,
  ...
  float* exchange_ptr;
  float* binned_output =
      (float*)malloc(max_ndms * (t_processed) * sizeof(float) / 2 + 1);
  float* binned_output_next =
      (float*)malloc(max_ndms * (t_processed) * sizeof(float) / 4);
  ...
  // printf("\nBin: %d, Mean: %f, Stddev: %f", 1, mean, stddev_orig),
  // fflush(stdout);
  ...
  for(dm_count = 0; dm_count < ndms[i]; dm_count++) {
    for(k = 0; k < remaining_time; k++) {
      if((output_buffer[remaining_time * dm_count + k] - mean) /
             (stddev_orig) >=
         cutoff) {
        //---------------- Modified ---------------
        if(OUTPUT_TO_FILE)
          fprintf(fp_out,
                  "%f, %f, %f, %d, %d\n",
                  ((float)k) * tsamp + start_time,
                  dm_low[i] + ((float)dm_count) * dm_step[i],
                  (output_buffer[remaining_time * dm_count + k] - mean) /
                      stddev_orig,
                  i,
                  1);
        //                                                      time       | DM
  ...
  for(dm_count = 0; dm_count < ndms[i]; dm_count++) {
    ...
    for(k = 0; k < remaining_time / 2; k++) {
      ...
      binned_output[shift + k] =
          ((output_buffer[shift3]) + (output_buffer[shift3 + 1])) * 0.5f;
    }
  }
  ...
  while(bin_factor > 1 && remaining_time > 1) {
    ...
    stddev =
        stddev_orig /
        ((float)sqrt(2.0f * powf(2, (counter - 1)))); // Stddev for data sample
    // Print mean and stddev
    // printf("\nBin: %d, Mean: %f, Stddev: %f", (int)powf(2,counter), mean,
    // stddev), fflush(stdout);

    // Apply threshold
    for(dm_count = 0; dm_count < ndms[i]; dm_count++) {
      for(k = 0; k < remaining_time; k++) {
        if((binned_output[remaining_time * dm_count + k] - mean) / (stddev) >=
               cutoff &&
           binned_output[remaining_time * dm_count + k] + (mean) > 0.0f) {
          //---------------- Modified ---------------
          if(OUTPUT_TO_FILE)
            fprintf(fp_out,
                    "%f, %f, %f, %d, %d\n",
                    ((float)k * taps) * tsamp + start_time,
                    dm_low[i] + ((float)dm_count) * dm_step[i],
                    (output_buffer[remaining_time * dm_count + k] - mean) /
                        stddev,
                    i,
    ...
    for(dm_count = 0; dm_count < ndms[i]; dm_count++) {
      ...
      for(k = 0; k < remaining_time / 2; k++) {
        int shift3 = 2 * k + shift2;
        binned_output_next[shift + k] =
            ((binned_output[shift3]) + (binned_output[shift3 + 1])) * 0.5f;
      }
    }
