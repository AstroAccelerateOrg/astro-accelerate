...
__global__ void SNR_GPU_limited(float*  d_FIR_input,
                                float*  d_SNR_output,
                                ushort* d_SNR_taps,
                                float*  d_MSD,
                                int     x_steps,
  ...
  for(int xf = 0; xf < x_steps; xf++) {
    if(pos_x < (nColumns - offset)) {
      ...
      // new_SNR = (d_FIR_input[pos_y + pos_x]-nTaps*signal_mean)/(sqrt((float)
      // nTaps)*signal_sd);
...
void call_kernel_SNR_GPU_limited(const dim3&   grid_size,
                                 const dim3&   block_size,
                                 float* const  d_FIR_input,
                                 float* const  d_SNR_output,
                                 ushort* const d_SNR_taps,
                                 float* const  d_MSD,
                                 const int&    x_steps,
                                 const int&    nTaps,
                                 const int&    nColumns,
                                 const int&    offset) {
  SNR_GPU_limited<<<grid_size, block_size>>>(d_FIR_input,
                                             d_SNR_output,
                                             d_SNR_taps,
                                             d_MSD,
                                             x_steps,
