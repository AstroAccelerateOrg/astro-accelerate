...
void acceleration(int      range,
  ...
  for(int i = 0; i < range; i++) {
    ...
    size = sizeof(float) * (samps / 2) * (2 * ACCMAX + ACCSTEP) / ACCSTEP;
    printf("\nSize of total host power signal:\t%zu MB", size / 1024 / 1024),
        fflush(stdout);
    ...
    // TEST:checkCudaErrors(cudaMemcpyAsync(d_signal_in_e,
    // output_buffer[i][230],   samps*sizeof(float), cudaMemcpyHostToDevice,
    // stream_e));
    checkCudaErrors(cudaMemcpyAsync(d_signal_in_e,
                                    output_buffer[i][0],
                                    samps * sizeof(float),
    ...
    for(a = 0; a < trials; a++) {
      ...
      stretch_gpu(event_e,
                  ...
                  tsamp,
                  d_signal_in_e,
                  d_signal_transformed_e);
      cudaStreamWaitEvent(stream_e, event_e, 0);
      checkCudaErrors(cufftExecR2C(plan_e,
                                   (float *)d_signal_transformed_e,
                                   (cufftComplex *)d_signal_fft_e));
    ...
    for(int dm_count = 1; dm_count < ndms[i] - 1; dm_count += 2) {
      ...
      checkCudaErrors(cudaMemcpyAsync(d_signal_in_o,
      ...
      for(a = 0; a < trials; a++) {
        ...
        stretch_gpu(event_o,
                    ...
                    tsamp,
                    d_signal_in_o,
                    d_signal_transformed_o);
        checkCudaErrors(cufftExecR2C(plan_o,
                                     (float *)d_signal_transformed_o,
                                     (cufftComplex *)d_signal_fft_o));
        cudaStreamWaitEvent(stream_o, event_o, 0);
        power_gpu(
            event_o, stream_o, samps, a, d_signal_fft_o, d_signal_power_o);
      }
      // cudaDeviceSynchronize();
      ...
      /*			for(a = 0; a < trials; a++) {
       ...
       { fprintf(fp_dm_e, "\n%d\t%f\t%f\t%f", acc, dm, j*((1.0f/tsamp)/(samps)),
       (float)(((double)h_signal_power_e[j+(a)*(samps/2)]-mean)/stddev));
       }
       }
       fprintf(fp_dm_e, "\n");
       }
      ...
      for(a = 0; a < trials; a++) {
        ...
        checkCudaErrors(cufftExecR2C(plan_e,
                                     ...
                                     (cufftComplex *)d_signal_fft_e));
        power_gpu(
            event_e, stream_e, samps, a, d_signal_fft_e, d_signal_power_e);
      }
      ...
      /*			for(a = 0; a < trials; a++) {
       ...
       { fprintf(fp_dm_o, "\n%d\t%f\t%f\t%f", acc, dm, j*((1.0f/tsamp)/(samps)),
       (float)(((double)h_signal_power_o[j+(a)*(samps/2)]-mean)/stddev));
       }
       }
       fprintf(fp_dm_o, "\n");
       }
