...
__global__ void power_kernel(int           half_samps,
  ...
  if(t < half_samps)
    d_signal_power[t + acc * (half_samps)] =
        (d_signal_fft[t + 1].x * d_signal_fft[t + 1].x +
         d_signal_fft[t + 1].y * d_signal_fft[t + 1].y);
}

//}}}

__global__ void
GPU_simple_power_and_interbin_kernel(float2 *d_input_complex,
  ...
  if((pos_x < (nTimesamples >> 1))) {
    d_output_power[blockIdx.y * (nTimesamples >> 1) + pos_x] =
        A.x * A.x + A.y * A.y;
    d_output_interbinning[blockIdx.y * nTimesamples + 2 * pos_x] =
        A.x * A.x + A.y * A.y;
    d_output_interbinning[blockIdx.y * nTimesamples + 2 * pos_x + 1] =
        0.616850275f * ((A.x - B.x) * (A.x - B.x) + (A.y - B.y) * (A.y - B.y));
  }
}
...
void call_kernel_power_kernel(const dim3 &        block_size,
  ...
  power_kernel<<<block_size, grid_size, smem_bytes, stream>>>(
      half_samps, acc, d_signal_fft, d_signal_power);
}
...
void call_kernel_GPU_simple_power_and_interbin_kernel(
    ...
    const float & norm) {
  GPU_simple_power_and_interbin_kernel<<<grid_size, block_size>>>(
      d_input_complex,
      d_output_power,
