...
__global__ void PD_SEARCH_GPU(float const *__restrict__ d_input,
                              float *d_output,
                              float *d_output_taps,
                              float *d_MSD,
                              int    maxTaps,
...
void call_kernel_PD_SEARCH_GPU(const dim3 &       grid_size,
  ...
  PD_SEARCH_GPU<<<grid_size, block_size, sm_size>>>(
      d_input, d_output, d_output_taps, d_MSD, maxTaps, nTimesamples);
}
