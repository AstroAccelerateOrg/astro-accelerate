...
void print_usage() {
  ...
  printf("\n        ----------------------------------        \n\n");
  printf("The program reads an accelerated sinusoid or optionally a pulse with "
         "a number of harmonics,\nand performs a (single harmonic) fourier "
         "domain acceleration search on this signal on a GPU\n");
  printf("\nUsage:  fdas [OPTION] \n\n");
  printf("Note:These command line options do not include all useful "
         "parameters.\nTo change correlation and  hardware related parameters "
         "you can edit the file params.h\n");
  printf("\n OPTIONS:\n\n");
  ...
  printf(" %-13s %-2s %-.55s\n",
         "-nname [name]",
         ": ",
         "Noise data file name (with -wsig)\n");
  printf(" %-13s %-2s %-.55s\n %-16s %-.55s\n %-16s %-.55s\n",
         "-nharm [n]",
         ":",
         "Number of harmonics to add in signal (default=1).",
         "",
         "Only used with -wsig, it inserts the specified number",
         "",
         "of harmonics to the signal created\n");
  printf(" %-13s %-2s %-.55s\n %-16s %-.55s\n%-16s %-.55s\n",
         "-nsig [n]",
         ":",
         "Noise multiplier (integer, default is 0, i.e. no noise).",
         " ",
         "Currently only use when a noise input file is available",
         "",
         " under the 'data' directory.\n");
  printf(" %-13s %-2s %-.55s\n",
         "-duty [f]",
         ":",
         "Duty cucle of pulse (%) (default is  10).\n");
  printf(" %-13s %-2s %-.60s\n %-16s %-.60s\n",
         "-iter [n]",
         ":",
         "Number of iterations to run the acceleration search.",
         " ",
         "(default is 1).\n");
  printf(" %-13s %-2s %-.60s\n",
         "-writef [0/1]",
         ":",
         "Write results to file for plots (default is no (0)).\n");
  printf(" %-13s %-2s %-.60s\n %-16s %-.55s\n",
         "-zval [n]",
         ":",
         "Value of z in bins drifted when creating a signal file",
         "",
         "(default is 4).\n");
  ...
  printf(" %-13s %-2s %-.60s\n %-16s %-.55s\n",
         "-mul [n]",
         ":",
         "Multiplier for non-power of two signal lengths,",
         "",
         "multiplies by 8192 to get the signal length (default is 1024).\n");
  printf(" %-13s %-2s %-.60s\n %-16s %-.55s\n",
         "-wsig",
         ":",
         "Create and write a time series to binary file",
         "",
         "- use to create signal files (default is no).\n");
  printf(" %-13s %-2s %-.60s\n",
         "-search",
         ":",
         "Perform the full search loop (default is no).\n");
  printf(" %-13s %-2s %-.60s\n",
         "-basic",
         ":",
         "Use with -search to search using the basic method.\n");

  printf(" %-13s %-2s %-.60s\n",
         "-kfft",
         ":",
         "Use with -search to search using custom FFT.\n");
  printf(" %-13s %-2s %-.60s\n",
         "-thresh [p]",
         ":",
         "Normalized power threshold, real number (default is 10.0).\n");
  printf(" %-13s %-2s %-.60s\n %-16s %-.55s\n",
         "-freq0 [f]",
         ":",
         "Central frequency of signal, real number.",
         "",
         "Used with -wsig. (default is 100.5).\n");
  printf(" %-13s %-2s %-.60s\n %-16s %-.55s\n",
         "-sigamp [f]",
         ":",
         "Amplitude of signal to create (only with -wsig)",
         "",
         "(default is 0.1).\n");
  printf(" %-13s %-2s %-.60s\n",
         "-devid [i]",
  ...
  printf(" %-13s %-2s %-.60s\n",
         "-inbin",
         ":",
         "Perform interbinning on the complex output.\n");
  printf(" %-13s %-2s %-.60s\n",
         "-norm",
         ":",
