...
__global__ void simple_corner_turn_kernel(float* d_input,
                                          float* d_output,
                                          int    primary_size,
                                          int    secondary_size) {
  ...
  d_output[primary * secondary_size + secondary] =
      (float)__ldg(&d_input[secondary * primary_size + primary]);
}

__global__ void corner_turn_SM_kernel(float const* __restrict__ d_input,
                                      float* d_output,
                                      int    primary_size,
                                      int    secondary_size) {
  ...
  gpos = (blockIdx.y * (blockDim.x >> 5) + warp_id) * CT_ROWS_PER_WARP *
             primary_size +
         blockIdx.x * CT_CORNER_BLOCKS * WARP + local_id;
...
__global__ void simple_corner_turn_kernel(unsigned short* d_input,
                                          float*          d_output,
                                          int             nchans,
                                          int             nsamp) {
...
__global__ void
swap(unsigned short* d_input, float* d_output, int nchans, int nsamp) {
...
void call_kernel_simple_corner_turn_kernel(const dim3&           block_size,
  ...
  simple_corner_turn_kernel<<<block_size, grid_size>>>(
      d_input, d_output, primary_size, secondary_size);
}
...
void call_kernel_corner_turn_SM_kernel(const dim3&        grid_size,
                                       ...
                                       const int&         primary_size,
                                       const int&         secondary_size) {
  corner_turn_SM_kernel<<<grid_size, block_size>>>(
      d_input, d_output, primary_size, secondary_size);
}
