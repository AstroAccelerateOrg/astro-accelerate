...
void main_function(int   argc,
  ...
  if(enable_analysis == 1) {
    stratagy_MSD(max_ndms,
                 ...
                 &h_MSD_DIT_width);
    allocate_memory_MSD(&d_MSD_workarea,
                        &d_MSD_output_taps,
                        &d_MSD_interpolated,
                        MSD_data_info,
                        h_MSD_DIT_width,
  ...
  // float *out_tmp;
  // out_tmp = (float *) malloc(( t_processed[0][0] + maxshift ) * max_ndms *
  // sizeof(float)); memset(out_tmp, 0.0f, t_processed[0][0] + maxshift *
  // max_ndms * sizeof(float));
  ...
  for(t = 0; t < num_tchunks; t++) {
    ...
    load_data(-1,
              inBin,
              d_input,
              &input_buffer[(long int)(inc * nchans)],
              t_processed[0][t],
              maxshift,
              nchans,
              dmshifts);
    ...
    for(dm_range = 0; dm_range < range; dm_range++) {
      ...
      if(inBin[dm_range] > oldBin) {
        bin_gpu(d_input,
                d_output,
                nchans,
                t_processed[dm_range - 1][t] + maxshift * inBin[dm_range]);
        (tsamp) = (tsamp)*2.0f;
      }
      ...
      dedisperse(dm_range,
                 ...
                 d_output,
                 nchans,
                 (t_processed[dm_range][t] + maxshift),
                 maxshift,
                 &tsamp,
                 dm_low,
                 dm_high,
      ...
      if((enable_acceleration == 1) || (enable_periodicity == 1) ||
         (analysis_debug == 1)) {
        // gpu_outputsize = ndms[dm_range] * ( t_processed[dm_range][t] ) *
        // sizeof(float);
        // save_data(d_output, out_tmp, gpu_outputsize);

        //#pragma omp parallel for
        for(int k = 0; k < ndms[dm_range]; k++) {
          // memcpy(&output_buffer[dm_range][k][inc / inBin[dm_range]],
          // &out_tmp[k * t_processed[dm_range][t]], sizeof(float) *
          // t_processed[dm_range][t]);

          save_data_offset(d_output,
                           k * t_processed[dm_range][t],
                           output_buffer[dm_range][k],
                           inc / inBin[dm_range],
      ...
      if(enable_analysis == 1) {
        ...
        if(analysis_debug == 1) {
          float* out_tmp;
          gpu_outputsize =
              ndms[dm_range] * (t_processed[dm_range][t]) * sizeof(float);
          out_tmp = (float*)malloc((t_processed[0][0] + maxshift) * max_ndms *
                                   sizeof(float));
          memset(out_tmp,
                 0.0f,
                 t_processed[0][0] + maxshift * max_ndms * sizeof(float));
          save_data(d_output, out_tmp, gpu_outputsize);
          analysis_CPU(dm_range,
                       tstart_local,
                       t_processed[dm_range][t],
                       (t_processed[dm_range][t] + maxshift),
                       nchans,
          ...
          size_t        peak_pos;
          max_peak_size =
              (size_t)(ndms[dm_range] * t_processed[dm_range][t] / 2);
          h_peak_list_DM =
              (unsigned int*)malloc(max_peak_size * sizeof(unsigned int));
          h_peak_list_TS =
              (unsigned int*)malloc(max_peak_size * sizeof(unsigned int));
    ...
    // memset(out_tmp, 0.0f, t_processed[0][0] + maxshift * max_ndms *
    // sizeof(float));
  ...
  printf("\n\n === OVERALL DEDISPERSION THROUGHPUT INCLUDING SYNCS AND DATA "
         "TRANSFERS ===\n");
  ...
  if(d_MSD_interpolated != NULL)
    cudaFree(d_MSD_interpolated);
  if(d_MSD_workarea != NULL)
    cudaFree(d_MSD_workarea);
  if(d_MSD_output_taps != NULL)
  ...
  printf("\nGops based on %.2lf ops per channel per tsamp: %f",
         NOPS,
         ((NOPS * all_processed) / (time)) / 1000000000.0);
  int   num_reg          = SNUMREG;
  ...
  float size_gb =
      (nchans * (t_processed[0][0]) * sizeof(float) * 8) / 1000000000.0;
  ...
  if(enable_acceleration == 1) {
    ...
    GpuTimer timer;
    timer.Start();
    // acceleration(range, nsamp, max_ndms, inc, nboots, ntrial_bins, navdms,
    // narrow, wide, nsearch, aggression, sigma_cutoff, output_buffer, ndms,
    // inBin, dm_low, dm_high, dm_step, tsamp_original);
