...
void statistics(char *       string,
  ...
  if(strcmp(string, "dedisperse in") == 0) {
    ...
    *out_time = clock();
    ...
    printf("\nGops based on %d ops per channel per tsamp: %lf",
           NOPSLOOP,
           ((NOPSLOOP * all_processed) / (time)) / 1000000000.0);
    printf("\nDevice global memory bandwidth in GB/s: %f",
           (((divisions_in_t * divisions_in_dm * num_blocks_t * num_blocks_dm) *
             nchans * sizeof(float)) /
            (time)) /
               1000000000);

    float num_threads = total_ndms * (nsamp - maxshift) / (num_reg);
    float data_size_loaded =
        (num_threads * nchans * sizeof(float)) / 1000000000;
