...
void Create_list_of_boxcar_widths2(std::vector<int> *boxcar_widths,
...
void stratagy_MSD(int                nDMs,
  ...
  printf("  Memory required by boxcar filters:%0.3f MB\n",
         (4.5 * vals * sizeof(float) + 2 * vals * sizeof(ushort)) /
             (1024.0 * 1024));
  printf("  Memory available:%0.3f MB \n",
         ((float)free_mem) / (1024.0 * 1024.0));
  *maxtimesamples =
      (free_mem * 0.95) / ((5.5 * sizeof(float) + 2 * sizeof(ushort)));
  ...
  *MSD_nDIT_widths = (int)(max_boxcar_width_in_sec / tsamp);
  ...
  //        cudaMalloc((void **) &d_MSD_interpolated,
  //        MSD_profile_size_in_bytes); cudaMalloc((void **)
