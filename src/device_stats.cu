...
void stats_gpu(cudaEvent_t  event,
               cudaStream_t stream,
               int          samps,
               float*       mean,
               float*       stddev,
  ...
  /*
   // Calculate the mean
   double	total=0.0;
   #pragma omp parallel for default(shared) private(a,j) schedule(static,chunk)
   reduction(+:total) for(a = 0; a < trials; a++) { for(j=0;j< (samps/2);j++){
   total += ((double)(h_signal_power[j+(a)*(samps/2)]));
   //		if(h_signal_power[j+(a)*(samps/2)] > 2465757) printf("\n%d %d
   %f", a, j, h_signal_power[j+(a)*(samps/2)]);
   }
   }
   *mean = (float)(total/(double)((samps/2)*(trials+1)));  // Mean for data
   sample
   ...
   reduction(+:total) for(a = 0; a < trials; a++) { for(j=0; j<(samps/2); j++){
   total +=
   (double)((h_signal_power[j+(a)*(samps/2)]-(*mean))*(h_signal_power[j+(a)*(samps/2)]-(*mean)));
   }
   }
   *stddev = (float)sqrt(abs(total) / (double)((samps/2)*(trials+1))); // Stddev
   for data sample
   */
  ...
  call_kernel_stats_kernel(num_blocks,
                           threads_per_block,
                           0,
                           stream,
                           half_samps,
  ...
  checkCudaErrors(cudaMemcpyAsync(
      h_sum, d_sum, size * sizeof(float), cudaMemcpyDeviceToHost, stream));
  checkCudaErrors(cudaMemcpyAsync(h_sum_square,
                                  d_sum_square,
                                  size * sizeof(float),
  ...
  *stddev = (float)sqrt((total_sum_square - acc_size * (*mean) * (*mean)) /
                        (acc_size - 1));
  // printf("\nM:\t%f, S:\t%f", *mean, *stddev);
