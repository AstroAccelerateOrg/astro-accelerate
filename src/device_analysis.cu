...
void Create_list_of_boxcar_widths(std::vector<int>* boxcar_widths,
...
void Create_PD_plan(std::vector<PulseDetection_plan>* PD_plan,
                    std::vector<int>*                 BC_widths,
                    int                               nDMs,
                    int                               nTimesamples) {
  ...
  if(BC_widths->size() > 0) {
    ...
    if(PDmp.decimated_timesamples < PDmp.unprocessed_samples)
      PDmp.nBlocks = 0;
    ...
    for(int f = 1; f < (int)BC_widths->size(); f++) {
      ...
      if(nRest > 0)
        PDmp.nBlocks++;
      PDmp.unprocessed_samples =
          PDmp.unprocessed_samples / 2 + PDmp.nBoxcars + 6; //
      if(PDmp.decimated_timesamples < PDmp.unprocessed_samples)
        PDmp.nBlocks = 0;
...
int Get_max_iteration(int               max_boxcar_width,
                      std::vector<int>* BC_widths,
...
void analysis_GPU(unsigned int*     h_peak_list_DM,
  ...
  //----------> GPU part
  printf("\n----------> GPU analysis part\n");
  printf("  Dimensions nDMs:%d; nTimesamples:%d; inBin:%d; outBin:%d; "
         "maxshift:%d; \n",
         ndms[i],
         t_processed,
         inBin,
  ...
  std::vector<int> BC_widths(t_BC_widths,
                             t_BC_widths + sizeof(t_BC_widths) / sizeof(int));
  ...
  if((int)DM_list.size() > 1)
    printf("  SPS will run %d batches each containing %d DM trials. Remainder "
           "%d DM trials\n",
           (int)DM_list.size(),
           DMs_per_cycle,
           nRest);
  ...
  max_iteration = Get_max_iteration(
      max_boxcar_width / inBin, &BC_widths, &max_width_performed);
  printf("  Selected iteration:%d; maximum boxcar width requested:%d; maximum "
         "boxcar width performed:%d;\n",
         max_iteration,
         max_boxcar_width / inBin,
         max_width_performed);
  Create_PD_plan(&PD_plan, &BC_widths, 1, nTimesamples);
  std::vector<int> h_boxcar_widths;
  Create_list_of_boxcar_widths(
      &h_boxcar_widths, &BC_widths, max_width_performed);

//-------------------------------------------------------------------------
//---------> Comparison between interpolated values and computed values
#ifdef MSD_BOXCAR_TEST
  MSD_plane_profile_boxcars(output_buffer,
                            nTimesamples,
                            nDMs,
                            &h_boxcar_widths,
                            OR_sigma_multiplier,
  ...
  MSD_plane_profile(d_MSD_interpolated,
                    output_buffer,
                    d_MSD_DIT,
                    d_MSD_workarea,
                    false,
  ...
  if(DM_list.size() > 0) {
    ...
    for(int f = 0; f < DM_list.size(); f++) {
      //-------------- SPDT
      timer.Start();
      SPDT_search_long_MSD_plane(&output_buffer[DM_shift * nTimesamples],
                                 d_boxcar_values,
                                 d_decimated,
                                 d_output_SNR,
                                 d_output_taps,
      ...
      printf("    BC_shift:%d; DMs_per_cycle:%d; f*DMs_per_cycle:%d; "
             "max_iteration:%d;\n",
             DM_shift * nTimesamples,
             DM_list[f],
             DM_shift,
             max_iteration);
#endif
      ...
      checkCudaErrors(cudaMemcpy(
          &temp_peak_pos, gmem_peak_pos, sizeof(int), cudaMemcpyDeviceToHost));
#ifdef GPU_ANALYSIS_DEBUG
      ...
      if(temp_peak_pos >= local_max_list_size) {
        printf("    Maximum list size reached! Increase list size or increase "
               "sigma cutoff.\n");
        temp_peak_pos = local_max_list_size;
      }
      if(((*peak_pos) + temp_peak_pos) < max_peak_size) {
        checkCudaErrors(cudaMemcpy(&h_peak_list_DM[(*peak_pos)],
                                   d_peak_list_DM,
                                   temp_peak_pos * sizeof(unsigned int),
                                   cudaMemcpyDeviceToHost));
        checkCudaErrors(cudaMemcpy(&h_peak_list_TS[(*peak_pos)],
                                   d_peak_list_TS,
                                   temp_peak_pos * sizeof(unsigned int),
                                   cudaMemcpyDeviceToHost));
        checkCudaErrors(cudaMemcpy(&h_peak_list_SNR[(*peak_pos)],
                                   d_peak_list_SNR,
                                   temp_peak_pos * sizeof(float),
                                   cudaMemcpyDeviceToHost));
        checkCudaErrors(cudaMemcpy(&h_peak_list_BW[(*peak_pos)],
                                   d_peak_list_BW,
                                   temp_peak_pos * sizeof(unsigned int),
                                   cudaMemcpyDeviceToHost));
...
#pragma omp parallel for
    for(int count = 0; count < (*peak_pos); count++) {
      h_peak_list[4 * count] =
          ((double)h_peak_list_DM[count]) * dm_step[i] + dm_low[i];
      h_peak_list[4 * count + 1] =
          ((double)h_peak_list_TS[count]) * tsamp + tstart;
  ...
  printf(
      "  MSD_time: %f ms; SPDT time: %f ms; Candidate selection time: %f ms;\n",
      MSD_time,
      SPDT_time,
      PF_time);
  printf("----------<\n\n");
