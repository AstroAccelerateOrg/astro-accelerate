...
void stratagy(int *   maxshift,
  ...
  *dmshifts = (float *)malloc(nchans * sizeof(float));
  ...
  if(power != 2.0) {
    ...
    for(c = 0; c < nchans; c++) {
      (*dmshifts)[c] =
          (float)(4148.741601f *
                  ((1.0 / pow((double)(fch1 + (foff * c)), power)) -
                   (1.0 / pow((double)fch1, power))));
    }
  }

  for(i = 0; i < range; i++) {
    modff((((int)((user_dm_high[i] - user_dm_low[i]) / user_dm_step[i]) +
            SDIVINDM) /
           SDIVINDM),
          &n); // This calculates number of SDIVINDM blocks per DM range
    (*ndms)[i] =
        (int)((int)n * SDIVINDM); // This is number of DM trial per DM range
    if(*max_ndms < (*ndms)[i])
      *max_ndms = (*ndms)[i]; // looking for maximum number of DM trials for
                              // memory allocation
    *total_ndms = *total_ndms + (*ndms)[i];
  }
  printf("\nMaximum number of dm trials in any of the range steps:\t%d",
         *max_ndms);

  (*dm_low)[0] = user_dm_low[0]; //
  (*dm_high)[0] =
      (*dm_low)[0] +
      ((*ndms)[0] * (user_dm_step[0])); // Redefines DM plan to suit GPU
  ...
  for(i = 1; i < range; i++) {
    ...
    if(inBin[i - 1] > 1) {
      *maxshift =
          (int)ceil((((*dm_low)[i - 1] + (*dm_step)[i - 1] * (*ndms)[i - 1]) *
                     (*dmshifts)[nchans - 1]) /
                    (tsamp));
      *maxshift =
          (int)ceil((float)(*maxshift + ((float)(SDIVINT * 2 * SNUMREG))) /
                    (float)inBin[i - 1]) /
          (float)(SDIVINT * 2 * SNUMREG);
  ...
  if(inBin[range - 1] > 1) {
    *maxshift = (int)ceil(
        (((*dm_low)[range - 1] + (*dm_step)[range - 1] * (*ndms)[range - 1]) *
         (*dmshifts)[nchans - 1]) /
        (tsamp));
    *maxshift =
        (int)ceil((float)(*maxshift + ((float)(SDIVINT * 2 * SNUMREG))) /
  ...
  *maxshift = (maxshift_high + (SNUMREG * 2 * SDIVINT));
  printf("\nRange:\t%d, MAXSHIFT:\t%d, Scrunch value:\t%d",
         range - 1,
         *maxshift,
         inBin[range - 1]);
  ...
  if(*maxshift >= nsamp) {
    printf("\n\nERROR!! Your maximum DM trial exceeds the number of samples "
  ...
  printf(
      "\nDiagonal DM:\t%f",
      (tsamp * nchans * 0.0001205 * powf((fch1 + (foff * (nchans / 2))), 3.0)) /
          (-foff * nchans));
  if(*maxshift >= nsamp) {
    printf("ERROR!! Your maximum DM trial exceeds the number of samples you "
           "have.\nReduce your maximum DM trial");
    exit(1);
  }
  ...
  if(nchans < (*max_ndms)) {
    ...
    // max_tsamps = (unsigned int) ( (*gpu_memory) / ( sizeof(unsigned short) *
    // ( (*max_ndms) + nchans ) ) ); // maximum number of timesamples we can fit
    // into GPU memory
    size_t SPDT_memory_requirements =
        (enable_analysis == 1 ? (sizeof(float) * (*max_ndms) * SPDT_fraction)
                              : 0);
    max_tsamps =
        (unsigned int)((*gpu_memory) /
                       (sizeof(unsigned short) * nchans +
                        sizeof(float) * (*max_ndms) +
                        SPDT_memory_requirements)); // maximum number of
                                                    // timesamples we can fit
                                                    // into GPU memory

    // Check that we dont have an out of range maxshift:
    if((*maxshift) > max_tsamps) {
      printf("\nERROR!! Your GPU doens't have enough memory for this number of "
             "dispersion trials.");
    ...
    if(nsamp < max_tsamps) {
      ...
      unsigned long int local_t_processed = (unsigned long int)floor(
          ((double)(nsamp - (*maxshift)) / (double)inBin[range - 1]) /
          (double)(SDIVINT * 2 * SNUMREG)); // number of timesamples per block
      local_t_processed =
          local_t_processed * (SDIVINT * 2 * SNUMREG) * inBin[range - 1];
      for(i = 0; i < range; i++) {
        (*t_processed)[i] =
            (int *)malloc(sizeof(int)); // TODO: change to size_t
        (*t_processed)[i][0] =
            (int)floor(((float)(local_t_processed) / (float)inBin[i]) /
                       (float)(SDIVINT * 2 * SNUMREG));
      ...
      // Work out how many blocks of time samples we need to complete the
      // processing upto nsamp-maxshift
      // int num_blocks = (int) floor(( (float) nsamp - ( *maxshift ) )) / (
      // (float) ( samp_block_size ) ) + 1;

      // Find the common integer amount of samples between all bins
      int local_t_processed =
          (int)floor(((float)(samp_block_size) / (float)inBin[range - 1]) /
                     (float)(SDIVINT * 2 * SNUMREG));
      local_t_processed =
          local_t_processed * (SDIVINT * 2 * SNUMREG) * inBin[range - 1];

      int num_blocks = (int)floor(((float)nsamp - (float)(*maxshift))) /
      ...
      remainder     = (int)floor((float)remainder / (float)inBin[range - 1]) /
                  (float)(SDIVINT * 2 * SNUMREG);
      ...
      for(i = 0; i < range; i++) {
        ...
        // fractional bit
        (*t_processed)[i][num_blocks] =
            (int)floor(((float)(remainder) / (float)inBin[i]) /
                       (float)(SDIVINT * 2 * SNUMREG));
        (*t_processed)[i][num_blocks] =
            (*t_processed)[i][num_blocks] * (SDIVINT * 2 * SNUMREG);
      }
    ...
    // Maximum number of samples we can fit in our GPU RAM is then given by:
    // max_tsamps = (unsigned int) ( ( *gpu_memory ) / ( nchans * (
    // sizeof(float) + 2 * sizeof(unsigned short) ) ) );
    size_t SPDT_memory_requirements =
        (enable_analysis == 1 ? (sizeof(float) * (*max_ndms) * SPDT_fraction)
                              : 0);
    max_tsamps =
        (unsigned int)((*gpu_memory) /
                       (nchans * (sizeof(float) + sizeof(unsigned short)) +
                        SPDT_memory_requirements));
    ...
    if(nsamp < max_tsamps) {
      ...
      // Allocate memory to hold the values of nsamps to be processed
      int local_t_processed =
          (int)floor(((float)(nsamp - (*maxshift)) / (float)inBin[range - 1]) /
                     (float)(SDIVINT * 2 * SNUMREG));
      local_t_processed =
          local_t_processed * (SDIVINT * 2 * SNUMREG) * inBin[range - 1];
      for(i = 0; i < range; i++) {
      ...
      // samp_block_size was not used to calculate remainder instead there is
      // local_t_processed which might be different
      int num_blocks = (int)floor(((float)nsamp - (float)(*maxshift)) /
                                  ((float)local_t_processed));
  ...
  printf("\nMaxshift memory needed:\t%lu MB",
         nchans * (*maxshift) * sizeof(unsigned short) / 1024 / 1024);
  if(nchans < (*max_ndms)) {
