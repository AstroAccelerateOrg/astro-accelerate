...
/* Again this funtion uses a pointer to the file pointer so that it can update
 * the position of the file pointer :) Note the brakets surrounding the
 * input_buffer, these are needed due to operator percidence...
...
void get_recorded_data(FILE**           fp,
  ...
  if(nbits == 32) {
    ...
    // Calculate the bin size in a distribution of unsigned shorts for the input
    // data.
    ...
    // Read in the data, scale to an unsigned short range, transpose it and
    // store it in the input buffer
    total_data = 0;
    while(!feof(*fp)) {
      if(fread(temp_buffer, sizeof(float), nchans, *fp) != nchans)
        break;
      for(c = 0; c < nchans; c++) {
        (*input_buffer)[c + total_data * (nchans)] =
            (unsigned short)((temp_buffer[c] - min) / bin);
      }
      total_data++;
    }
    free(temp_buffer);
    ...
    unsigned char* temp_buffer =
        (unsigned char*)malloc(nb_bytes * sizeof(unsigned char));
    // Read in the data, transpose it and store it in the input buffer
    total_data = 0;
    // 00001111
    char mask = 0x0f;
    while(!feof(*fp)) {
      ...
      for(c = 0; c < nb_bytes; c++) {
        // (n >> a) & ( (1 << a) - 1) -> right shift by 'a' bits, then keep the
        // last 'b' bits Here, we right shift 4 bits and keep the last 4 bits
        (*input_buffer)[(c * 2) + total_data * (nchans)] =
            (unsigned short)((temp_buffer[c] >> 4) & mask);
        // n & ( (1 << a ) - 1)
        // Here we keep the last 4 bits
        (*input_buffer)[(c * 2) + 1 + total_data * (nchans)] =
            (unsigned short)(temp_buffer[c] & mask);
      }
      total_data++;
    }
    free(temp_buffer);
    ...
    while(!feof(*fp)) {
      if(fread(temp_buffer, sizeof(unsigned char), nb_bytes, *fp) != nb_bytes)
        break;
      for(c = 0; c < nb_bytes; c++) {
        (*input_buffer)[(c * 4) + total_data * (nchans)] =
            (unsigned short)((temp_buffer[c] >> 6) & mask);
        (*input_buffer)[(c * 4) + 1 + total_data * (nchans)] =
            (unsigned short)((temp_buffer[c] >> 4) & mask);
        (*input_buffer)[(c * 4) + 2 + total_data * (nchans)] =
            (unsigned short)((temp_buffer[c] >> 2) & mask);
        (*input_buffer)[(c * 4) + 3 + total_data * (nchans)] =
            (unsigned short)(temp_buffer[c] & mask);
      }
      total_data++;
    }
    ...
    while(!feof(*fp)) {
      ...
      for(c = 0; c < nb_bytes; c++) {
        for(int i = 0; i < 8; i++) {
          ...
          (*input_buffer)[(c * 8) + i + total_data * (nchans)] =
              (unsigned short)bit;
        }
      }
      total_data++;
