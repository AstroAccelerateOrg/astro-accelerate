...
void stretch_gpu(cudaEvent_t  event,
                 cudaStream_t stream,
                 int          acc,
                 int          samps,
                 float        tsamp,
  ...
  float t_zero =
      ((double)tsamp) / (1.0 + ((acc * samps * (double)tsamp) / 599584916.0));
  ...
  call_kernel_stretch_kernel(num_blocks,
                             threads_per_block,
                             0,
                             stream,
                             acc,
  ...
  // printf("\nCT Gops based on %.2f ops per channel per tsamp:
  // %f",10.0,((10.0*(divisions_in_t*divisions_in_f*num_blocks_t*num_blocks_f))/(time))/1000000000.0);
  // printf("\nCT Device memory bandwidth in GB/s: %f",
  // ((sizeof(float)+sizeof(unsigned
