...
/*
  ...
  f_line_bin[idx+(BINDIVINT*BINDIVINF)] = input_buffer[(c*in_nsamp)+(blockIdx.x
  * BINDIVINT*2)+idx+(BINDIVINT*BINDIVINF)];
  __syncthreads();
  ...
  bin_buffer[(shift_one)] = (f_line_bin[(shift_three)] + f_line_bin[shift_three
  + 1])/2;
  }
*/
...
__global__ void bin(unsigned short *d_input, float *d_output, int in_nsamp) {
  ...
  d_output[(shift_one)] =
      (float)((d_input[(shift_two)] + d_input[shift_two + 1]) / 2.0f);
}

__global__ void DiT_GPU_v2(float const *__restrict__ d_input,
                           float *      d_output,
                           unsigned int nDMs,
...
void call_kernel_bin(const dim3 &          num_blocks,
...
void call_kernel_DiT_GPU_v2(const dim3 &        gridSize,
                            const dim3 &        blockSize,
                            float const *const  d_input,
                            float *const        d_output,
                            const unsigned int &nDMs,
                            const unsigned int &nTimesamples,
                            const unsigned int &dts) {
  DiT_GPU_v2<<<gridSize, blockSize>>>(
      d_input, d_output, nDMs, nTimesamples, (nTimesamples >> 1));
}
