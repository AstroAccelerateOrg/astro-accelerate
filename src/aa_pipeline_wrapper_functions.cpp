...
namespace astroaccelerate {
  ...
  void dedisperse_analyse_telescope_data(const aa_filterbank_metadata &filterbank_data,
    ...
    aa_pipeline_generic(selected_modules, filterbank_data, dm_ranges, input_data, output_data, sigma_cutoff, sigma_constant, max_boxcar_width_in_sec, enable_candidate_algorithm, enable_sps_baseline_noise_algorithm);
  }
  ...
  void dedisperse_analyse_telescope_data(const aa_filterbank_metadata &filterbank_data,
					 ...
					 const bool &enable_sps_baseline_noise_algorithm) {
    // This overloaded function uses a std::vector<unsigned short> and calls the function version using a raw pointer to the data.
    dedisperse_analyse_telescope_data(filterbank_data, dm_ranges, input_data.data(), output_data, sigma_cutoff, sigma_constant, max_boxcar_width_in_sec, enable_candidate_algorithm, enable_sps_baseline_noise_algorithm);
  }
