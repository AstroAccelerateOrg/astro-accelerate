...
size_t Calculate_sd_per_file_from_file_size(size_t size_in_MB,
      ...
      for(int i = 0; i < (int)chunk_size.size(); i++) {
        ...
        for(size_t d = 0; d < chunk_size[i]; d++) {
          for(size_t t = 0; t < nTimesamples; t++) {
            h_data[d * nTimesamples + t] =
                dedispersed_data[r][inner_DM_shift + d][t];
          }
        }
...
void Export_data_raw(float *     h_data,
                     size_t      primary_dimension,
  ...
  if(sd_per_file < 0)
    sd_per_file = secondary_dimension;
  ...
  std::vector<int> chunk_size;
  for(int f = 0; f < nRepeats; f++)
    chunk_size.push_back(sd_per_file);
  if(nRest > 0)
    chunk_size.push_back(nRest);
  ...
  for(int i = 0; i < (int)chunk_size.size(); i++) {
    ...
    for(size_t d = 0; d < chunk_size[i]; d++) {
      for(size_t t = 0; t < primary_dimension; t++) {
        h_export_data[d * primary_dimension + t] =
            h_data[(inner_sec_dim_shift + d) * primary_dimension + t];
      }
    }
    ...
    fwrite(h_export_data,
           primary_dimension * chunk_size[i] * sizeof(float),
           1,
           fp_out);
    fclose(fp_out);
...
void Export_data_as_list(float *     h_data,
                         size_t      primary_dimension,
                         float       prim_mul,
                         float       prim_add,
                         size_t      secondary_dimension,
  ...
  for(int i = 0; i < (int)chunk_size.size(); i++) {
    ...
    for(size_t sec = 0; sec < chunk_size[i]; sec++) {
      for(size_t prim = 0; prim < primary_dimension; prim++) {
        h_data_to_export[3 * (sec * primary_dimension + prim)] =
            ((float)sec) * sec_mul + sec_add;
        h_data_to_export[3 * (sec * primary_dimension + prim) + 1] =
            ((float)prim) * prim_mul + prim_add;
        h_data_to_export[3 * (sec * primary_dimension + prim) + 2] =
            h_data[sec * primary_dimension + prim];
      }
    }
