...
int SPDT_threshold(float*                            d_input,
                   ushort*                           d_input_taps,
  ...
  for(int f = 0; f < max_iteration; f++) {
    ...
    if((decimated_timesamples - local_offset) > 0) {
      ...
      nRest =
          (decimated_timesamples - local_offset) - nBlocks * Elements_per_block;
      if(nRest > 0)
        nBlocks++;
      ...
      gridSize.x  = nCUDAblocks_x;
      gridSize.y  = nCUDAblocks_y;
      gridSize.z  = 1;
      blockSize.x = WARP * THR_WARPS_PER_BLOCK;
      blockSize.y = 1;
      blockSize.z = 1;

      output_offset = nDMs * PD_plan->operator[](f).output_shift;

      call_kernel_THR_GPU_WARP(gridSize,
                               blockSize,
                               &d_input[output_offset],
                               &d_input_taps[output_offset],
                               d_output_list_DM,
                               d_output_list_TS,
                               d_output_list_SNR,
                               d_output_list_BW,
                               ...
                               decimated_timesamples - local_offset,
                               shift,
                               max_list_size,
                               (1 << f));
...
int Threshold_for_periodicity_old(float*  d_input,
  ...
  if((primary_size % (blockSize.x * THR_ELEM_PER_THREAD)) != 0)
    nBlocks_p++;

  nBlocks_s = (int)(secondary_size / blockSize.y);
  if((secondary_size % blockSize.y) != 0)
    nBlocks_s++;
  ...
  THR_init();
  call_kernel_GPU_Threshold_for_periodicity_kernel_old(gridSize,
                                                       blockSize,
                                                       d_input,
                                                       d_input_harms,
                                                       d_output_list,
...
int Threshold_for_periodicity(float*  d_input,
                              ushort* d_input_harms,
                              float*  d_output_list,
                              int*    gmem_pos,
                              float*  d_MSD,
  ...
  for(size_t f = 0; f < secondary_size_per_chunk.size(); f++)
    printf("%zu ", secondary_size_per_chunk[f]);
  printf("\n");
#endif
  ...
  for(int f = 0; f < (int)secondary_size_per_chunk.size(); f++) {
    ...
    if((secondary_size_per_chunk[f] % blockSize.y) != 0)
      nBlocks_s++;
    ...
    call_kernel_GPU_Threshold_for_periodicity_kernel(
        gridSize,
        blockSize,
        &d_input[shift * primary_size],
        d_input_harms,
        d_output_list,
        gmem_pos,
        d_MSD,
