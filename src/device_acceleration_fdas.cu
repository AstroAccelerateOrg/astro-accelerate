...
*/
  ...
  // prepare signal
  params.offset = presto_z_resp_halfwidth(
      (double)ZMAX,
      0); // array offset when we pick signal points for the overlp-save method
  ...
  params.extlen =
      params.nblocks *
      KERNLEN; // signal array extended to array of separate N=KERNLEN segments
  params.ffdotlen = params.siglen *
                    NKERN; // total size of ffdot complex plane in fourier bins
  params.ffdotlen_cpx =
      params.extlen * NKERN; // total size of ffdot powers plane in fourier bins
  ...
  if(cmdargs.search) {
    printf("\nnumber of convolution templates NKERN = %d, template length = "
           "%d, acceleration step in bins = %f, zmax = %d, template size power "
           "of 2 = %d, scale=%f \n",
           NKERN,
           KERNLEN,
           ACCEL_STEP,
    ...
    size_t mem_ffdot =
        params.ffdotlen *
        sizeof(float); // total size of ffdot plane powers in bytes
    size_t mem_ffdot_cpx =
        params.ffdotlen_cpx *
        sizeof(float2); // total size of ffdot plane powers in bytes
    size_t mem_kern_array    = KERNLEN * NKERN * sizeof(float2);
    size_t mem_max_list_size = params.max_list_length * 4 * sizeof(float);
    size_t mem_signals       = (params.nsamps * sizeof(float)) +
                         (params.siglen * sizeof(float2)) +
    ...
    if(cmdargs.basic)
      mem_tot_needed = mem_ffdot + mem_ffdot_cpx + mem_kern_array +
                       mem_signals +
                       mem_max_list_size; // KA added + mem_max_list_size
    if(cmdargs.kfft)
    ...
    printf("Total memory for this device: %.2f GB\nAvailable memory on this "
           "device for data upload: %.2f GB \n",
           mtotal / gbyte,
           mfree / gbyte);
    ...
    printf("Total memory needed on GPU for arrays to process 1 DM: %.4f "
           "GB\nfloat ffdot plane (for power spectrum) = %.4f GB.\nTemplate "
           "array %.4f GB\nOne dimensional signals %.4f\n1 GB = %f",
           (double)mem_tot_needed / gbyte,
           (double)(mem_ffdot) / gbyte,
           (double)mem_kern_array / gbyte,
           (double)mem_signals / gbyte,
           gbyte);
    //
    if(mem_tot_needed >= (mfree - gbyte / 10)) {
      printf("\nNot enough memory available on the device to process this "
     ...
     * cudaStream_t *stream_list = malloc(number_dm_concurrently *
     * sizeof(cudaStream_t)); for (int ii = 0; ii < number_dm_concurrently;
     * ++ii)
     * {
     ...
     *
     * -- a list of arrays
     * gpuarrays *gpuarrays_list = malloc(number_dm_concurrently *
     * sizeof(gpuarrays));
     * -- allocate memory -> create a function which uses cudaMallocHost to
     * alloc pinned memory for (int ii = 0; ii < number_dm_concurrently; ++ii)
     * {
    ...
    printf("\nCreating acceleration templates with KERNLEN=%d, NKERN = %d "
           "zmax=%d... ",
           KERNLEN,
           NKERN,
           ZMAX);
...
//! TEST!: put test signal here
        ...
        for(int f = 0; f < processed; f++)
          output_buffer[i][dm_count][f] = rand() / (float)RAND_MAX;
        ...
        accel = ((double)acc_sig.zval * light_speed) /
                (acc_sig.freq0 * tobs * tobs);
        printf("\n\npreparing test signal, observation time = %f s, %d nsamps "
               "f0 = %f Hz with %d harmonics\n",
               tobs,
               acc_sig.nsamps,
               acc_sig.freq0,
               acc_sig.nharms);
        ...
        printf("\nNow creating accelerated signal with fc=%f, accel=%f, "
               "harmonics=%d, duty cycle=%.1f%, noise=%d signal samples=%d, "
               "signal level: %.2f\n",
               acc_sig.freq0,
               accel,
               acc_sig.nharms,
        ...
        for(int sd = 0; sd < acc_sig.nsamps; ++sd) {
          ...
          for(int j = 1; j <= acc_sig.nharms; ++j) {
            output_buffer[i][dm_count][sd] +=
                (2.0 / (j * M_PI) * sin(j * M_PI * acc_sig.duty)) *
                acc_sig.sigamp * cos(j * omega * tau);
          }
        }
        ...
        checkCudaErrors(cudaMemcpy(gpuarrays.d_in_signal,
                                   output_buffer[i][dm_count],
                                   processed * sizeof(float),
                                   cudaMemcpyHostToDevice));

        /*
         * checkCudaErrors( cudaMemcpyAsync(gpuarrays_list[i].d_in_signal,
         * output_buffer[i][dm_count], processed*sizeof(float),
         * cudaMemcpyHostToDevice, stream_list[ii]));
         *
         */

        cudaDeviceSynchronize();
        gettimeofday(&t_end, NULL);
        t_gpu = (double)(t_end.tv_sec + (t_end.tv_usec / 1000000.0) -
                         t_start.tv_sec - (t_start.tv_usec / 1000000.0)) *
                1000.0;
        t_gpu_i = (t_gpu / (double)titer);
        printf("\n\nAverage vector transfer time of %d float samples (%.2f Mb) "
               "from 1000 iterations: %f ms\n\n",
               params.nsamps,
               (float)(gpuarrays.mem_insig) / mbyte,
               t_gpu_i);
        ...
        if(cmdargs.basic || cmdargs.kfft) {
          ...
          float*       d_MSD;
          float        h_MSD[3];
          if(cudaSuccess != cudaMalloc((void**)&d_MSD, sizeof(float) * 3))
            printf("Allocation error!\n");
          unsigned int* gmem_fdas_peak_pos;
          if(cudaSuccess !=
             cudaMalloc((void**)&gmem_fdas_peak_pos, 1 * sizeof(int)))
            printf("Allocation error!\n");
          ...
          if(NKERN >= 32) {
            printf("Block\n");
            MSD_grid_outlier_rejection(d_MSD,
...
//! TEST!: do not perform peak find instead export the thing to file.
#ifdef FDAS_CONV_TEST
          fdas_write_test_ffdot(
              &gpuarrays, &cmdargs, &params, dm_low[i], dm_count, dm_step[i]);
          exit(1);
#endif
          //! TEST!: do not perform peak find instead export the thing to file.

          PEAK_FIND_FOR_FDAS(gpuarrays.d_ffdot_pwr,
          ...
          checkCudaErrors(cudaMemcpy(
              h_MSD, d_MSD, 3 * sizeof(float), cudaMemcpyDeviceToHost));
          checkCudaErrors(cudaMemcpy(&list_size,
                                     gmem_fdas_peak_pos,
                                     sizeof(unsigned int),
                                     cudaMemcpyDeviceToHost));

#ifdef FDAS_ACC_SIG_TEST
          fdas_write_list(&gpuarrays,
                          &cmdargs,
                          &params,
                          h_MSD,
                          dm_low[i],
  ...
  if(cmdargs.search) {
     ...
     * -- don't forget it's pinned memory here so write a function which uses
     * cudaFreeHost for (int ii = 0; ii < number_dm_concurrently; ++ii)
     * {
