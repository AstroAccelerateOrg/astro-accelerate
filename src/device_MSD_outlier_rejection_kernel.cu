...
__global__ void MSD_BLN_pw_rejection_normal(float const* __restrict__ d_input,
                                            float* d_output,
                                            float* d_MSD,
                                            int    y_steps,
                                            int    nTimesamples,
...
__global__ void MSD_BLN_grid_outlier_rejection_new(float* d_input,
                                                   float* d_output,
                                                   int    size,
                                                   float  multiplier) {
  ...
  for(int f = 0; f < 5; f++) {
    ...
    if(size > blockDim.x) {
      ...
      while(pos < size) {
        ...
        if(((int)jv) != 0) {
          if((Mt / jv > (signal_mean - multiplier * signal_sd)) &&
             (Mt / jv < (signal_mean + multiplier * signal_sd))) {
            if((int)j == 0) {
              M = Mt;
...
__global__ void
MSD_BLN_grid_calculate_partials(float const* __restrict__ d_input,
                                float* d_output,
                                int    x_steps,
                                int    y_steps,
                                int    nColumns,
  ...
  pos = (blockIdx.y * dim_y + warp_id) * y_steps * nColumns +
        blockIdx.x * WARP * x_steps + local_id;
  M = __ldg(&d_input[pos]);
  S = 0;
  j = 1.0f;
  ...
  for(int i = (blockDim.x >> 1); i > HALF_WARP; i = i >> 1) {
    ...
    // in the last iteration we do not need to save the results... or
    // synchronize threads...
    __syncthreads();
  }
...
__global__ void
MSD_BLN_grid_outlier_rejection(float const* __restrict__ d_input,
                               float* d_output,
                               int    size,
                               float  nElements,
                               float  multiplier) {
  ...
  for(int f = 0; f < 5; f++) {
    ...
    if(size > blockDim.x) {
      ...
      while(pos < size) {
        Mt = __ldg(&d_input[3 * pos]);
        if((Mt / nElements > (signal_mean - multiplier * signal_sd)) &&
           (Mt / nElements < (signal_mean + multiplier * signal_sd))) {
          if(j == 0) {
            M = Mt;
            S = __ldg(&d_input[3 * pos + 1]);
            j = nElements;
      ...
      // by now we should have only 32 partial results. shuffle reduction
      // follows
      for(int q = HALF_WARP; q > 0; q = q >> 1) {
        jv = __shfl_down(j, q);
        if(jv != 0) {
    ...
    // if(threadIdx.x==0) printf("Corrected mean:%f; and standard
    // deviation:%f;\n", signal_mean, signal_sd);
  }
  //---- Iterations with outlier rejection
  //----------------------------------------------
...
void call_kernel_MSD_BLN_pw_rejection_normal(const dim3&        grid_size,
                                             ...
                                             const int&         offset,
                                             const float& bln_sigma_constant) {
  MSD_BLN_pw_rejection_normal<<<grid_size, block_size>>>(d_input,
                                                         d_output,
                                                         d_MSD,
                                                         y_steps,
                                                         nTimesamples,
...
void call_kernel_MSD_BLN_grid_outlier_rejection_new(const dim3&  grid_size,
                                                    const dim3&  block_size,
                                                    float* const d_input,
                                                    float* const d_output,
                                                    const int&   size,
                                                    const float& multiplier) {
  MSD_BLN_grid_outlier_rejection_new<<<grid_size, block_size>>>(
      d_input, d_output, size, multiplier);
}

void call_kernel_MSD_BLN_grid_calculate_partials(const dim3&        grid_size,
                                                 const dim3&        block_size,
                                                 const int&         threads,
                                                 float const* const d_input,
                                                 float* const       d_output,
                                                 ...
                                                 const int&         nColumns,
                                                 const int&         msd) {
  MSD_BLN_grid_calculate_partials<<<grid_size, block_size, threads>>>(
      d_input, d_output, x_steps, y_steps, nColumns, msd);
}

void call_kernel_MSD_BLN_grid_outlier_rejection(const dim3&        grid_size,
                                                const dim3&        block_size,
                                                float const* const d_input,
                                                float* const       d_output,
                                                const int&         size,
                                                const float&       nElements,
                                                const float&       multiplier) {
  MSD_BLN_grid_outlier_rejection<<<grid_size, block_size>>>(
      d_input, d_output, size, nElements, multiplier);
}
