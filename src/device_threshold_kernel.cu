...
__global__ void THR_GPU_WARP(float const *__restrict__ d_input,
                             ushort *      d_input_taps,
                             unsigned int *d_output_list_DM,
                             unsigned int *d_output_list_TS,
                             float *       d_output_list_SNR,
                             unsigned int *d_output_list_BW,
                             int *         gmem_pos,
                             float         threshold,
                             int           nTimesamples,
                             int           offset,
    ...
    if(pos_x < offset) {
      ...
      if(R > threshold) {
        ...
        if(list_pos < max_list_size) {
          d_output_list_DM[list_pos] =
              blockIdx.y * THR_WARPS_PER_BLOCK + warp_id + shift;
          d_output_list_TS[list_pos] =
              DIT_value * pos_x + d_input_taps[(pos_x + pos_y)] / 2;
          d_output_list_SNR[list_pos] = R;
          d_output_list_BW[list_pos] =
              (unsigned int)d_input_taps[(pos_x + pos_y)];
        }
      }
    }
    pos_x = pos_x + WARP;
...
__global__ void
GPU_Threshold_for_periodicity_kernel_old(float const *__restrict__ d_input,
                                         ushort *d_input_harms,
                                         float * d_output_list,
                                         int *   gmem_pos,
                                         float * d_MSD,
                                         float   threshold,
    ...
    if((pos_p < primary_size) && (pos_s < secondary_size)) {
      ...
      if(R > threshold) {
        ...
        if(list_pos < max_list_size) {
          ...
          d_output_list[4 * list_pos + 2] =
              inverse_white_noise(&R, &hrms, &mean, &sd);
        }
      }
      //-------------------------<
    }
...
__global__ void
GPU_Threshold_for_periodicity_kernel(float const *__restrict__ d_input,
                                     ushort *d_input_harms,
                                     float * d_output_list,
                                     int *   gmem_pos,
    ...
    if((pos_p < primary_size) && (pos_s < secondary_size)) {
      ...
      if(R > threshold) {
        ...
        if(list_pos < max_list_size) {
          ...
          d_output_list[4 * list_pos + 3] = hrms;
          d_output_list[4 * list_pos + 2] =
              R * __ldg(&d_MSD[2 * hrms + 1]) + __ldg(&d_MSD[2 * hrms]);
        }
      }
      //-------------------------<
    }
...
void call_kernel_THR_GPU_WARP(const dim3 &        grid_size,
                              ...
                              const int &         DIT_value) {
  THR_GPU_WARP<<<grid_size, block_size>>>(d_input,
                                          d_input_taps,
                                          d_output_list_DM,
                                          d_output_list_TS,
                                          d_output_list_SNR,
...
void call_kernel_GPU_Threshold_for_periodicity_kernel_old(
    ...
    const int &        max_list_size,
    const int &        DIT_value) {
  GPU_Threshold_for_periodicity_kernel_old<<<grid_size, block_size>>>(
      d_input,
      d_input_harms,
      d_output_list,
      gmem_pos,
...
void call_kernel_GPU_Threshold_for_periodicity_kernel(
    const dim3 &       grid_size,
    const dim3 &       block_size,
    float const *const d_input,
    ushort *const      d_input_harms,
    ...
    const int &        max_list_size,
    const int &        DIT_value) {
  GPU_Threshold_for_periodicity_kernel<<<grid_size, block_size>>>(
      d_input,
      d_input_harms,
      d_output_list,
      gmem_pos,
