...
void fdas_print_params_h() {
  ...
  printf("\nNumber of templates including zero acceleration: NKERN %d\n",
         NKERN);
  //  printf("\nLowest acceleration in fourier bins (z) (for harmonic sum): ZLO
  //  %d\n", ZLO);
  printf("\nThread block size in x direction for 2-D thread block convolution "
         "GPU kernels : TBSIZEX %d\n",
         TBSIZEX);
  printf("\nThread block size in Y direction for 2-D thread block convolution "
         "GPU kernels : TBSIZEY %d\n",
         TBSIZEY);
  printf("\nThread block size in x direction for 2-D thread block power "
         "spectrum GPU kernels : PTBSIZEX %d\n",
         PTBSIZEX);
  printf("\nThread block size in y direction for 2-D thread block power "
         "spectrum GPU kernels : PTBSIZEY %d\n",
         PTBSIZEY);
...
void fdas_alloc_gpu_arrays(fdas_gpuarrays* arrays, cmd_args* cmdargs) {
  ...
  // templates
  checkCudaErrors(
      cudaMalloc((void**)&arrays->d_kernel, KERNLEN * sizeof(float2) * NKERN));
  ...
  // Added by KA
  if(cudaSuccess !=
     cudaMalloc((void**)&arrays->d_fdas_peak_list, arrays->mem_max_list_size))
    printf("Allocation error in FDAS: d_fdas_peak_list\n");

  // check allocated/free memory
  size_t mfree, mtotal;
  checkCudaErrors(cudaMemGetInfo(&mfree, &mtotal));
  printf("\nMemory allocation finished: Total memory for this device: %.2f "
...
/*
{
  ...
  printf("\n\npreparing test signal, observation time = %f s, %d nsamps f0 = %f
Hz with %d harmonics\n", tobs, acc_sig->nsamps, acc_sig->freq0,
acc_sig->nharms); printf("\nz = %d accelereation = %f m/s^2\n", acc_sig->zval,
accel); acc_sig->acc_signal = (float*)malloc(acc_sig->nsamps*sizeof(float));

  printf("\nNow creating accelerated signal with fc=%f, accel=%f, harmonics=%d,
duty cycle=%.1f%, noise=%d signal samples=%d, signal level: %.2f\n",
acc_sig->freq0, accel, acc_sig->nharms, acc_sig->duty*100.0, cmdargs->nsig,
acc_sig->nsamps,acc_sig->sigamp);
  ...
  for ( int i=0; i<acc_sig->nsamps; ++i){
    ...
    for (int j = 1; j <= acc_sig->nharms; ++j){
      acc_sig->acc_signal[i] +=
(2.0/(j*M_PI)*sin(j*M_PI*acc_sig->duty))*acc_sig->sigamp*cos(j*omega*tau);
    }
  }
  //Write to file
  char afname[200];
  sprintf(afname, "data/acc_sig_8192x%d_%dharms_%dduty_%.3fHz_%dz_%dnsigma.dat",
acc_sig->mul,  acc_sig->nharms, (int)( acc_sig->duty*100.0),  acc_sig->freq0,
acc_sig->zval, acc_sig->nsig );
...
void fdas_create_acc_kernels(cufftComplex* d_kernel, cmd_args* cmdargs) {
  /* Create kernel templates for the correlation technique (Ransom et. al.
   * 2002),  */
  ...
  for(ii = 0; ii < NKERN; ii++) {
    ...
    presto_place_complex_kernel(
        tempkern, numkern, (h_kernel + ii * KERNLEN), KERNLEN);
    free(tempkern);
  }

//! TEST!: replace templates here. Template width: numkern; padded width:
//! KERNLEN
  ...
  for(ii = 0; ii < NKERN; ii++) {
    ...
    for(int f = 0; f < KERNLEN; f++) {
      ...
      if(f < boxcar_width / 2)
        h_kernel[ii * KERNLEN + f].x = 1.0;
      if(f >= (KERNLEN - boxcar_width / 2))
        h_kernel[ii * KERNLEN + f].x = 1.0;
    }
  }
#endif
  ...
  checkCudaErrors(cudaMemcpy(d_kernel,
                             h_kernel,
                             KERNLEN * sizeof(float2) * NKERN,
                             cudaMemcpyHostToDevice)); // upload kernels to GPU
...
void fdas_cuda_create_fftplans(fdas_cufftplan* fftplans, fdas_params* params) {
  ...
  checkCudaErrors(
      cufftEstimate1d(params->nsamps, CUFFT_R2C, 1, &real_worksize));
  printf("\nsignal real fft plan requires extra %f MB of memory\n",
         real_worksize / mbyte);

  // estimate plan memory for forward fft
  checkCudaErrors(cufftEstimateMany(nrank,
                                    n,
                                    inembed,
                                    istride,
                                    idist,
  ...
  printf("\nsignal forward fft plan requires extra  %f MB of memory\n the same "
         "plan is used for the inverse fft",
         sig_worksize / mbyte);
  ...
  checkCudaErrors(cufftMakePlanMany(fftplans->realplan,
                                    nrank,
                                    rn,
                                    rinembed,
                                    istride,
  ...
  checkCudaErrors(cufftMakePlanMany(fftplans->forwardplan,
                                    nrank,
                                    n,
                                    inembed,
                                    istride,
...
// real fft
  ...
  cufftExecR2C(
      fftplans->realplan, gpuarrays->d_in_signal, gpuarrays->d_fft_signal);
#endif
  ...
  checkCudaErrors(cudaMemcpy(ftemp,
                             gpuarrays->d_in_signal,
                             (params->rfftlen) * sizeof(float),
                             cudaMemcpyDeviceToHost));
  for(int f = 0; f < params->rfftlen; f++) {
    f2temp[f].x = ftemp[f];
    f2temp[f].y = 0;
  }
  checkCudaErrors(cudaMemcpy(gpuarrays->d_fft_signal,
                             f2temp,
                             (params->rfftlen) * sizeof(float2),
  ...
  if(cmdargs->norm) {
    ...
    checkCudaErrors(cudaMemcpy(fftsig,
                               gpuarrays->d_fft_signal,
                               (params->rfftlen) * sizeof(float2),
                               cudaMemcpyDeviceToHost));
  ...
  // overlap-copy
  cuda_overlap_copy<<<KERNLEN / 64, 64>>>(gpuarrays->d_ext_data,
                                          gpuarrays->d_fft_signal,
                                          params->sigblock,
                                          params->rfftlen,
                                          params->extlen,
  ...
  if(cmdargs->norm) {
    ...
    checkCudaErrors(cudaMemcpy(extsig,
                               gpuarrays->d_ext_data,
                               (params->extlen) * sizeof(float2),
                               cudaMemcpyDeviceToHost));
    for(int b = 0; b < params->nblocks; ++b)
      presto_norm(extsig + b * KERNLEN, KERNLEN);
    checkCudaErrors(cudaMemcpy(gpuarrays->d_ext_data,
                               extsig,
                               (params->extlen) * sizeof(float2),
                               cudaMemcpyHostToDevice));
    free(extsig);
  ...
  // complex block fft
  cufftExecC2C(fftplans->forwardplan,
               gpuarrays->d_ext_data,
               gpuarrays->d_ext_data,
               CUFFT_FORWARD);
  ...
  cuda_convolve_reg_1d_halftemps<<<cblocks, cthreads>>>(gpuarrays->d_kernel,
                                                        gpuarrays->d_ext_data,
                                                        gpuarrays->d_ffdot_cpx,
                                                        params->extlen,
                                                        params->scale);
  ...
  for(int k = 0; k < ZMAX / 2; k++) {
    cufftExecC2C(fftplans->forwardplan,
                 gpuarrays->d_ffdot_cpx + k * params->extlen,
                 gpuarrays->d_ffdot_cpx + k * params->extlen,
                 CUFFT_INVERSE);
    cufftExecC2C(fftplans->forwardplan,
                 gpuarrays->d_ffdot_cpx + (ZMAX - k) * params->extlen,
                 gpuarrays->d_ffdot_cpx + (ZMAX - k) * params->extlen,
                 CUFFT_INVERSE);
  }
  // z=0
  cufftExecC2C(fftplans->forwardplan,
               gpuarrays->d_ffdot_cpx + ((ZMAX / 2) * params->extlen),
               gpuarrays->d_ffdot_cpx + ((ZMAX / 2) * params->extlen),
               CUFFT_INVERSE);

  // power spectrum
  if(cmdargs->inbin) {
    cuda_ffdotpow_concat_2d_inbin<<<pwblocks, pwthreads>>>(
        gpuarrays->d_ffdot_cpx,
...
// real fft
  ...
  // overlap-copy
  cuda_overlap_copy_smallblk<<<params->nblocks, KERNLEN>>>(
      gpuarrays->d_ext_data,
      gpuarrays->d_fft_signal,
      params->sigblock,
      params->rfftlen,
  ...
  if(cmdargs->inbin) {
    cuda_convolve_customfft_wes_no_reorder02_inbin<<<params->nblocks,
                                                     KERNLEN>>>(
        gpuarrays->d_kernel,
        gpuarrays->d_ext_data,
        gpuarrays->d_ffdot_pwr,
    ...
    GPU_CONV_kFFT_mk11_2elem_2v<<<gridSize,blockSize>>>(gpuarrays->d_ext_data,
    gpuarrays->d_ffdot_pwr, gpuarrays->d_kernel, params->sigblock,
    params->offset, params->nblocks, params->scale);
    */
    ...
    GPU_CONV_kFFT_mk11_4elem_2v<<<gridSize, blockSize>>>(gpuarrays->d_ext_data,
                                                         gpuarrays->d_ffdot_pwr,
                                                         gpuarrays->d_kernel,
                                                         params->sigblock,
                                                         params->offset,
...
void fdas_write_list(fdas_gpuarrays* gpuarrays,
  ...
  if(!isnan(h_MSD[0]) || !isinf(h_MSD[0]) || !isnan(h_MSD[1]) ||
     !isinf(h_MSD[1])) {
    ...
    checkCudaErrors(cudaMemcpy(h_fdas_peak_list,
                               gpuarrays->d_fdas_peak_list,
                               list_size * 4 * sizeof(float),
                               cudaMemcpyDeviceToHost));
    ...
    if((fp_c = fopen(pfname, "w")) == NULL) {
      fprintf(stderr,
              "Error opening %s file for writing: %s\n",
              pfname,
              strerror(errno));
    }
...
void fdas_write_ffdot(fdas_gpuarrays* gpuarrays,
  ...
  // download data
  checkCudaErrors(cudaMemcpy(h_ffdotpwr,
                             gpuarrays->d_ffdot_pwr,
                             params->ffdotlen * sizeof(float),
                             cudaMemcpyDeviceToHost));
  ...
  float        pow, sigma;
  double       tobs = (double)params->tsamp * (double)params->nsamps * ibin;
  unsigned int numindep =
      params->siglen * (NKERN + 1) * ACCEL_STEP / 6.95; // taken from PRESTO
  ...
  for(int a = 0; a < NKERN; a++) {
    ...
    for(int j = 0; j < ibin * params->siglen; j++) {
      pow = h_ffdotpwr[a * ibin * params->siglen +
                       j]; //(h_ffdotpwr[a * params->siglen + j]-mean)/stddev;

      if(pow > cmdargs->thresh) {
        sigma = candidate_sigma(pow,
                                cmdargs->nharms,
                                numindep); // power, number of harmonics, number
                                           // of independed searches=1...2^harms
        //  sigma=1.0;
