...
void dedisperse(int             i,
  ...
  if(failsafe == 0) {
    if(nbits == 16 || nbits == 32) {
      ...
      int   shifta =
          (int)floorf(shift_one * dmshifts[nchans - 1]) + (SDIVINT - 1) * 2;
      ...
      if(((SDIVINT - 1) + ((SDIVINDM - 1) * SDIVINT) - 1) > lineshift) {
        ...
        // cudaFuncCachePreferShared); //Subsume in call_kernel_*
        call_kernel_shared_dedisperse_kernel_16(num_blocks,
                                                threads_per_block,
                                                inBin[i],
                                                d_input,
                                                d_output,
      ...
      if(((SDIVINT - 1) + ((SDIVINDM - 1) * SDIVINT) - 1) > lineshift) {
        ...
        cudaDeviceSetSharedMemConfig(cudaSharedMemBankSizeFourByte);
        // cudaFuncSetCacheConfig(shared_dedisperse_kernel,
        // cudaFuncCachePreferShared); //Subsume in call_kernel_*
        call_kernel_shared_dedisperse_kernel(num_blocks,
                                             threads_per_block,
                                             inBin[i],
                                             d_input,
                                             d_output,
  ...
  if(failsafe != 0) {
    ...
    // //Subsume in call_kernel_*
    call_kernel_cache_dedisperse_kernel(num_blocks,
                                        threads_per_block,
                                        inBin[i],
                                        d_input,
                                        d_output,
