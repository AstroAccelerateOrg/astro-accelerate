...
int PD_FIR(
    float* d_input, float* d_output, int nTaps, int nDMs, int nTimesamples) {
  //---------> Task specific
  ...
  int nCUDAblocks_x = (int)((nTimesamples - nTaps + 1) /
                            (PD_FIR_ACTIVE_WARPS * WARP * PD_FIR_NWINDOWS));
  ...
  //---------> Pulse detection FIR
  PD_FIR_init();
  call_kernel_PD_FIR_GPU(gridSize,
                         blockSize,
                         SM_size,
                         d_input,
                         d_output,
  ...
  ut = nTimesamples -
       nCUDAblocks_x * PD_FIR_ACTIVE_WARPS * WARP * PD_FIR_NWINDOWS;
  return (ut);
}

int GPU_FIRv1_wrapper(float*       d_input,
                      float*       d_output,
                      int          nTaps,
                      unsigned int nDMs,
  ...
  dim3 gridSize(
      nCUDAblocks_x, nCUDAblocks_y, 1); // nCUDAblocks_y goes through spectra
  dim3 blockSize(
      PD_FIR_ACTIVE_WARPS * WARP, 1, 1); // nCUDAblocks_x goes through channels
  ...
  cudaDeviceSetCacheConfig(cudaFuncCachePreferShared);
  cudaDeviceSetSharedMemConfig(cudaSharedMemBankSizeEightByte);
  call_kernel_PD_FIR_GPUv1(gridSize,
                           blockSize,
                           SM_size,
                           d_input,
                           d_output,
...
int PPF_L1(
    float* d_input, float* d_output, int nDMs, int nTimesamples, int nTaps) {
  //---------> CUDA block and CUDA grid parameters
  int itemp         = nTimesamples - nTaps + 1;
  int nCUDAblocks_x = (int)(itemp / PPF_L1_THREADS_PER_BLOCK);
  if(itemp % PPF_L1_THREADS_PER_BLOCK != 0)
    nCUDAblocks_x++;
