...
int main(int argc, char *argv[]) {
  ...
  // Users desired de-dispersion strategy. Pick up user defined values from the
  // CLI.
  get_user_input(&fp,
                 argc,
                 argv,
  ...
  // Store the recorded telescope data contained in the input filterbank file in
  ...
  if(output_buffer != 0 && output_buffer != NULL)
    free(output_buffer);
  free(t_processed);
  if(dm_low != 0 && dm_low != NULL)
    free(dm_low);
  if(dm_high != 0 && dm_high != NULL)
    free(dm_high);
  if(dm_step != 0 && dm_step != NULL)
    free(dm_step);
  if(dmshifts != 0 && dmshifts != NULL)
    free(dmshifts);
  if(user_dm_low != 0 && user_dm_low != NULL)
    free(user_dm_low);
  if(user_dm_high != 0 && user_dm_high != NULL)
    free(user_dm_high);
  if(user_dm_step != 0 && user_dm_step != NULL)
    free(user_dm_step);
  if(nb_selected_dm > 0) {
