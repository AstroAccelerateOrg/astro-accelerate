...
__global__ void dilate_peak_find(const float*  d_input,
                                 ushort*       d_input_taps,
  ...
  if(peak == 1) {
    ...
    if(list_pos < max_peak_size) {
      ...
      d_peak_list_TS[list_pos] =
          idxX * DIT_value +
          d_input_taps[idxY * width + idxX] / 2; // time coordinate (x)
      d_peak_list_SNR[list_pos] = my_value;      // SNR value
      d_peak_list_BW[list_pos] =
          d_input_taps[idxY * width + idxX]; // width of the boxcar
    }
  }
...
__global__ void dilate_peak_find_for_fdas(const float*  d_input,
                                          float*        d_peak_list,
                                          float*        d_MSD,
                                          const int     width,
                                          const int     height,
...
// width DM
// height time
__global__ void dilate_peak_find_for_periods_old(const float* d_input,
                                                 ushort*      d_input_taps,
                                                 float*       d_peak_list,
                                                 const int    width,
                                                 const int    height,
  ...
  if(peak == 1) {
    ...
    if(list_pos < max_peak_size) {
      ...
      d_peak_list[4 * list_pos + 1] =
          idxY / DIT_value; // frequency coordinate (x)
      float hrms                    = d_input_taps[idxY * width + idxX];
      d_peak_list[4 * list_pos + 3] = hrms; // width of the boxcar
      d_peak_list[4 * list_pos + 2] =
          inverse_white_noise(&my_value, &hrms, &mean, &sd); // SNR value
    }
  }
...
__global__ void dilate_peak_find_for_periods(const float* d_input,
                                             ushort*      d_input_taps,
                                             float*       d_peak_list,
                                             const int    width,
                                             const int    height,
  ...
  if(peak == 1) {
    ...
    if(list_pos < max_peak_size) {
      ...
      d_peak_list[4 * list_pos + 3] = hrms; // width of the boxcar
      d_peak_list[4 * list_pos + 2] = my_value * __ldg(&d_MSD[2 * hrms + 1]) +
                                      __ldg(&d_MSD[2 * hrms]); // SNR value
    }
  }
...
void call_kernel_dilate_peak_find(const dim3&         grid_size,
                                  const dim3&         block_size,
                                  float* const        d_input,
                                  ushort* const       d_input_taps,
                                  unsigned int* const d_peak_list_DM,
                                  unsigned int* const d_peak_list_TS,
                                  float* const        d_peak_list_SNR,
                                  unsigned int* const d_peak_list_BW,
                                  const int&          width,
                                  const int&          height,
                                  const int&          offset,
                                  const float&        threshold,
                                  ...
                                  const int&          shift,
                                  const int&          DIT_value) {
  dilate_peak_find<<<grid_size, block_size>>>(d_input,
                                              d_input_taps,
                                              d_peak_list_DM,
                                              d_peak_list_TS,
                                              d_peak_list_SNR,
...
void call_kernel_dilate_peak_find_for_fdas(const dim3&         grid_size,
                                           const dim3&         block_size,
                                           float* const        d_input,
                                           ...
                                           const float&        DM_trial) {
  dilate_peak_find_for_fdas<<<grid_size, block_size>>>(d_input,
                                                       d_peak_list,
                                                       d_MSD,
                                                       width,
                                                       height,
...
void call_kernel_dilate_peak_find_for_periods_old(const dim3&   grid_size,
                                                  const dim3&   block_size,
                                                  float* const  d_input,
                                                  ushort* const d_input_taps,
                                                  float* const  d_peak_list,
                                                  ...
                                                  const int&    DM_shift,
                                                  const int&    DIT_value) {
  dilate_peak_find_for_periods_old<<<grid_size, block_size>>>(d_input,
                                                              d_input_taps,
                                                              d_peak_list,
                                                              width,
                                                              height,
...
void call_kernel_dilate_peak_find_for_periods(const dim3&        grid_size,
                                              const dim3&        block_size,
                                              float* const       d_input,
                                              ushort* const      d_input_taps,
                                              float* const       d_peak_list,
                                              ...
                                              const int&         DM_shift,
                                              const int&         DIT_value) {
  dilate_peak_find_for_periods<<<grid_size, block_size>>>(d_input,
                                                          d_input_taps,
                                                          d_peak_list,
                                                          width,
                                                          height,
