...
int PD_SEARCH_INPLACE(float *        d_input,
  ...
  cudaMalloc((void **)&d_output,
             nBlocks_x * (maxTaps - 1) * nDMs * sizeof(float));
  ...
  call_kernel_PD_ZC_GPU_KERNEL(gridSize_temp,
                               blockSize_temp,
                               d_input,
                               d_output,
                               maxTaps,
  ...
  call_kernel_PD_INPLACE_GPU_KERNEL(gridSize,
                                    blockSize,
                                    SM_size,
                                    d_input,
                                    d_output,
