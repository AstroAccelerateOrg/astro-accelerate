...
//{{{ stretch
__global__ void stretch_kernel(int    acc,
                               int    samps,
                               float  tsamp,
                               float *d_input,
                               float *d_output,
...
void call_kernel_stretch_kernel(const dim3 &        block_size,
  ...
  stretch_kernel<<<block_size, grid_size, smem_bytes, stream>>>(
      acc, samps, tsamp, d_input, d_output, t_zero, multiplier, tsamp_inverse);
}
