...
// void cpu_blocked_bootstrap(float *data_array, unsigned int num_els, unsigned
// int num_bins, unsigned int num_boots, float *mean_data_extern, float
// *cpu_mean_extern, float *cpu_sd_extern){

// ************************************* GPU Blocked Bootstrap
// ************************************** //

// ************************************* Bootstrap Kernel
// ************************************** // most bias, but quickest (rand in
// (0,bins-blockDim.x))
__global__ void bootstrap(int           bins,
                          int           num_els,
                          int           num_boots,
                          float*        g_idata,
                          double*       g_odata,
                          unsigned int* g_irand) {
  ...
  g_odata[dmid + threadIdx.x + blockDim.x * blockIdx.x] =
      ((double)myResample / (double)num_els);
}

// some bias, slower than bootstrap (rand in (0,bins-1) but with modulo to loop)
__global__ void bootstrap2(int           bins,
                           int           num_els,
                           int           num_boots,
                           float*        g_idata,
                           double*       g_odata,
...
// least amount of bias, slower than bootstrap2 (rand array is much bigger, each
// thread chooses a sample independent of other threads).
__global__ void bootstrap3(int           bins,
                           int           num_els,
                           int           num_boots,
                           float*        g_idata,
                           double*       g_odata,
...
void gpu_blocked_bootstrap(float** d_idata,
                           int     dms_to_average,
                           int     num_els,
                           int     ndms,
                           int     num_bins,
                           int     num_boots,
                           double* mean_boot_out,
                           double* mean_data_out,
                           double* sd_boot_out) {
  ...
  /*
   ...
   random_bin_array[dm_count*num_bins +
   exclusive_random[dm_count][j]]=bin_array[dm_count*num_bins +j];
   }
   }
   cudaMemcpy(d_bin_array, random_bin_array, local_ndms*num_bins*sizeof(float),
  ...
  for(int dm_count = 0; dm_count < local_ndms; dm_count++) {
    for(int j = 0; j < num_bins; j++) {
      random_bin_array[dm_count * num_bins + exclusive_random[j]] =
          bin_array[dm_count * num_bins + j];
    }
  }
  cudaMemcpy(d_bin_array,
             random_bin_array,
             local_ndms * num_bins * sizeof(float),
             cudaMemcpyHostToDevice);
  free(exclusive_random);

  // create array of random numbers: we need num_subboots*num_bins random
  // numbers
  ...
  if(BSTRAP == 1)
    bootstrap<<<num_blocks_boot, threads_per_block_boot>>>(
        num_bins, num_els, num_boots, d_bin_array, d_odata, d_irand);
  if(BSTRAP == 2)
    bootstrap2<<<num_blocks_boot, threads_per_block_boot>>>(
        num_bins, num_els, num_boots, d_bin_array, d_odata, d_irand);
  if(BSTRAP == 3)
    bootstrap3<<<num_blocks_boot, threads_per_block_boot>>>(
        num_bins, num_els, num_boots, d_bin_array, d_odata, d_irand);
  ...
  cudaDeviceSynchronize();
  cudaMemcpy(boots_array,
             d_odata,
             local_ndms * num_boots * sizeof(double),
  ...
  for(int dm_count = 0; dm_count < local_ndms; dm_count++) {
    ...
    for(int i = 0; i < num_boots; i++) {
      sd_boots += (boots_array[dm_count * num_boots + i] - mean_boots) *
                  (boots_array[dm_count * num_boots + i] - mean_boots);
    }
