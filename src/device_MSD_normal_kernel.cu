...
// Computes partials for mean and standard deviation of the data with offset at
// the end PD_THREADS could be replaced it is not required to be #defined
__global__ void MSD_GPU_limited(float const* __restrict__ d_input,
                                float* d_output,
                                int    y_steps,
                                int    nTimesamples,
                                int    offset) {
...
void call_kernel_MSD_GPU_limited(const dim3&        grid_size,
                                 ...
                                 const int&         offset) {
  MSD_GPU_limited<<<grid_size, block_size>>>(
      d_input, d_output, y_steps, nTimesamples, offset);
}
