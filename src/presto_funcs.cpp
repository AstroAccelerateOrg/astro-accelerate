...
/* PRESTO defines */
#define NUMLOCPOWAVG \
  20 // Number of bins (total) to average for local power:  Must be an even
     // number (1/2 on each side).
#define DELTAAVGBINS \
  5 // Number of bins next to freq in question to ignore (on each side) when
    // determining local power.
#define NUMFINTBINS \
  16 // Number of bins on each side of the central frequency to sum for Fourier
     // interpolation (low accuracy)
...
/*       contain.                                                    */
{
  ...
  response = (cufftComplex*)malloc(numkern *
                                   sizeof(cufftComplex)); // gen_cvect(numkern);
  ...
  if(roffset < 1E-3) {
    response[numkern / 2].x =
        1 - 6.579736267392905746 * (tmp = roffset * roffset);
...
/*       contain.                                                    */
{
  ...
  startr = -(0.5 * z);
  startroffset =
      (startr < 0) ? 1.0 + modf(startr, &tmprl) : modf(startr, &tmprl);
  signz = (z < 0.0) ? -1 : 1;
  ...
  if(startroffset < 1E-3 && absz < 1E-3) {
    ...
    response[numkernby2].y +=
        startroffset * (M_PI - 0.5167712780049970029 * zz);
    response[numkernby2].x +=
        xx * (-6.579736267392905746 + 0.9277056288952613070 * zz);
...
void presto_place_complex_kernel(cufftComplex* kernel,
                                 int           numkernel,
                                 cufftComplex* result,
                                 int           numresult)
...
/*   'numresult' is the number of points in the result.  */
{
  ...
  // printf("\nmemcpy numkernel = %d  halfwidth=%d
  // numresult=%d\n",numkernel,halfwidth,numresult);
  memcpy(result, kernel + halfwidth, sizeof(cufftComplex) * halfwidth);
  //  printf("\nmemcpy 1st half ok\n");
  memcpy(
      result + numresult - halfwidth, kernel, sizeof(cufftComplex) * halfwidth);
  //  printf("\nmemcpy 2nd half ok\n");
}
