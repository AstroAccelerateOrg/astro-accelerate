...
void corner_turn(unsigned short* d_input,
                 float*          d_output,
                 int             nchans,
                 int             nsamp) {
  ...
  call_kernel_simple_corner_turn_kernel(
      num_blocks, threads_per_block, d_input, d_output, nchans, nsamp);
  ...
  printf("\nCT Gops based on %.2f ops per channel per tsamp: %f",
         10.0,
         ((10.0 *
           (divisions_in_t * divisions_in_f * num_blocks_t * num_blocks_f)) /
          (time)) /
             1000000000.0);
  printf("\nCT Device memory bandwidth in GB/s: %lf",
...
int corner_turn(float* d_input,
  ...
  call_kernel_simple_corner_turn_kernel(num_blocks,
                                        threads_per_block,
                                        d_input,
                                        d_output,
                                        primary_size,
...
int corner_turn_SM(float* d_input,
                   float* d_output,
                   int    primary_size,
                   int    secondary_size) {
  //---------> Task specific
  ...
  call_kernel_corner_turn_SM_kernel(
      gridSize, blockSize, d_input, d_output, primary_size, secondary_size);
