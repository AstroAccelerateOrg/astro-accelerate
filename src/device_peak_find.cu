...
void SPDT_peak_find(float*                            d_output_SNR,
  ...
  for(int f = 0; f < max_iteration; f++) {
    ...
    if((decimated_timesamples - local_offset - 1) > 0) {
      gridSize.x =
          1 + ((decimated_timesamples - local_offset - 1) / blockDim.x);
      gridSize.y = 1 + ((nDMs - 1) / blockDim.y);
      gridSize.z = 1;

      call_kernel_dilate_peak_find(
          gridSize,
          blockDim,
          &d_output_SNR[nDMs * PD_plan->operator[](f).output_shift],
          &d_output_taps[nDMs * PD_plan->operator[](f).output_shift],
          d_peak_list_DM,
          d_peak_list_TS,
          d_peak_list_SNR,
          d_peak_list_BW,
...
void PEAK_FIND_FOR_FDAS(float*        d_ffdot_plane,
                        float*        d_peak_list,
                        float*        d_MSD,
                        int           nDMs,
                        int           nTimesamples,
  ...
  dim3 blockDim(32, 2, 1);
  dim3 gridSize(
      1 + ((nTimesamples - 1) / blockDim.x), 1 + ((nDMs - 1) / blockDim.y), 1);

  call_kernel_dilate_peak_find_for_fdas(gridSize,
                                        blockDim,
                                        d_ffdot_plane,
                                        d_peak_list,
                                        d_MSD,
...
void Peak_find_for_periodicity_search_old(float*  d_input_SNR,
  ...
  call_kernel_dilate_peak_find_for_periods_old(gridSize,
                                               blockDim,
                                               d_input_SNR,
                                               d_input_harmonics,
                                               d_peak_list,
...
void Peak_find_for_periodicity_search(float*  d_input_SNR,
                                      ushort* d_input_harmonics,
                                      float*  d_peak_list,
                                      int     secondary_size,
                                      int     primary_size,
  ...
  for(int f = 0; f < (int)secondary_size_per_chunk.size(); f++) {
    ...
    call_kernel_dilate_peak_find_for_periods(gridSize,
                                             blockDim,
                                             &d_input_SNR[shift * primary_size],
                                             d_input_harmonics,
                                             d_peak_list,
                                             primary_size,
                                             secondary_size,
