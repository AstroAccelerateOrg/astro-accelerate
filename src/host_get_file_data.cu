...
#include <stdio.h>
/* Note we send in a pointer to the file pointer becuase this function needs to
...
void get_file_data(FILE** fp,
  ...
  if((*nbits) == 32) {
    ...
    *nsamp = total_data - 1;
    ...
    total_data = 0;
    while(!feof(*fp)) {
      if(((fread(temp_buffer, sizeof(unsigned short), (*nchans), *fp)) !=
          (*nchans)) &&
         (total_data == 0)) {
        fprintf(stderr, "\nError while reading file\n");
        exit(0);
    ...
    *nsamp = total_data - 1;
    ...
    total_data = 0;
    while(!feof(*fp)) {
      if(((fread(temp_buffer, sizeof(unsigned char), (*nchans), *fp)) !=
          (*nchans)) &&
         (total_data == 0)) {
        fprintf(stderr, "\nError while reading file\n");
        exit(0);
    ...
    *nsamp = total_data - 1;
    ...
    if((*nchans % 2) != 0) {
      printf("\nNumber of frequency channels must be a power of 2 with 4 bit "
             "data\n");
      exit(0);
    }
    ...
    total_data = 0;
    while(!feof(*fp)) {
      if(((fread(temp_buffer, sizeof(unsigned char), nb_bytes, *fp)) !=
          nb_bytes) &&
         (total_data == 0)) {
        fprintf(stderr, "\nError while reading file\n");
        exit(0);
    ...
    *nsamp = total_data - 1;
    ...
    //		if ((*nchans / 4) != 0)
    //		{
    //			printf("\nNumber of frequency channels must be divisible
    //by 8 with 1 bit data samples\n"); 			exit(0);
    //		}
