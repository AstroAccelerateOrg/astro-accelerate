...
void Do_MSD_normal(float *d_MSD,
                   float *d_input,
...
void Do_MSD_continuous(float *d_MSD,
                       float *d_input,
  ...
  MSD_Configuration conf(nTimesamples, nDMs, offset, 0);
  if(enable_outlier_rejection) {
    MSD_outlier_rejection_continuous(d_MSD,
                                     d_input,
                                     d_previous_partials,
                                     d_MSD_workarea,
                                     &conf,
...
inline void Do_MSD(float *d_MSD,
                   float *d_input,
                   float *d_previous_partials,
                   float *d_MSD_workarea,
                   int    nTimesamples,
                   ...
                   float  OR_sigma_multiplier,
                   int    enable_outlier_rejection,
                   bool   perform_continuous) {
  if(perform_continuous)
    Do_MSD_continuous(d_MSD,
                      d_input,
                      d_previous_partials,
                      d_MSD_workarea,
                      nTimesamples,
...
void MSD_plane_profile_debug(float *d_MSD,
                             int    DIT_value,
                             int    nTimesamples,
                             int    MSD_pos) {
  float h_MSD[MSD_RESULTS_SIZE];
  checkCudaErrors(cudaMemcpy(
      h_MSD, d_MSD, MSD_RESULTS_SIZE * sizeof(float), cudaMemcpyDeviceToHost));
  printf("    DiT:%d; nTimesamples:%d; decimated_timesamples:%d; MSD_pos: %d; "
         "MSD:[%f; %f; %f]\n",
         (int)DIT_value,
         (int)nTimesamples,
         (int)(nTimesamples >> 1),
...
void MSD_of_input_plane(float *           d_MSD_DIT,
                        std::vector<int> *h_MSD_DIT_widths,
                        float *           d_input_data,
                        float *           d_MSD_DIT_previous,
                        float *           d_sudy,
                        float *           d_lichy,
                        float *           d_MSD_workarea,
                        size_t            nTimesamples,
  ...
  if(high_memory) {
    ...
    timer.Start();
    Do_MSD(&d_MSD_DIT[MSD_RESULTS_SIZE],
           d_lichy,
           &d_MSD_DIT_previous[MSD_RESULTS_SIZE],
           d_MSD_workarea,
           decimated_timesamples,
           nDMs,
           nRest,
           OR_sigma_multiplier,
           enable_outlier_rejection,
...
#ifdef MSD_PLANE_DEBUG
    MSD_plane_profile_debug(
        &d_MSD_DIT[MSD_RESULTS_SIZE], DIT_value, decimated_timesamples, 1);
#endif
    ...
    timer.Start();
    Do_MSD_continuous(&d_MSD_DIT[MSD_RESULTS_SIZE],
                      d_lichy,
                      &d_MSD_DIT[2 * MSD_RESULTS_SIZE],
                      d_MSD_workarea,
                      decimated_timesamples,
                      nDMs_half,
                      nRest,
    ...
    timer.Start();
    nRest =
        GPU_DiT_v2_wrapper(d_lichy, d_sudy, nDMs_half, decimated_timesamples);
    timer.Stop();
    t_dit_time += timer.Elapsed();

    // second half of the decimation
    timer.Start();
    nRest = GPU_DiT_v2_wrapper(&d_input_data[nDMs_half * nTimesamples],
                               d_lichy,
                               nDMs_half,
                               nTimesamples);
    decimated_timesamples = (nTimesamples >> 1);
    ...
    timer.Start();
    nRest =
        GPU_DiT_v2_wrapper(d_lichy,
                           &d_sudy[nDMs_half * (decimated_timesamples >> 1)],
                           nDMs_half,
                           decimated_timesamples);
    timer.Stop();
    t_dit_time += timer.Elapsed();
  ...
  timer.Start();
  Do_MSD(&d_MSD_DIT[2 * MSD_RESULTS_SIZE],
         d_sudy,
         &d_MSD_DIT_previous[2 * MSD_RESULTS_SIZE],
         d_MSD_workarea,
         decimated_timesamples,
         nDMs,
         nRest,
...
#ifdef MSD_PLANE_DEBUG
  MSD_plane_profile_debug(
      &d_MSD_DIT[2 * MSD_RESULTS_SIZE], DIT_value, decimated_timesamples, 2);
#endif
  //----------------------------------------------------------------------------------------
  ...
  for(f = 3; f <= nDecimations; f++) {
    ...
    if(DIT_value <= max_width_performed && switch_to_boxcar == 0) {
      if(f % 2 == 0) {
        ...
        //                            in      out :|
        nRest =
            GPU_DiT_v2_wrapper(d_lichy, d_sudy, nDMs, decimated_timesamples);
        timer.Stop();
        t_dit_time += timer.Elapsed();
        ...
        timer.Start();
        Do_MSD(&d_MSD_DIT[f * MSD_RESULTS_SIZE],
               d_sudy,
               &d_MSD_DIT_previous[f * MSD_RESULTS_SIZE],
               d_MSD_workarea,
               decimated_timesamples,
               nDMs,
               nRest,
        ...
        //                            in      out :|
        nRest =
            GPU_DiT_v2_wrapper(d_sudy, d_lichy, nDMs, decimated_timesamples);
        timer.Stop();
        t_dit_time += timer.Elapsed();
...
#ifdef MSD_PLANE_DEBUG
      MSD_plane_profile_debug(&d_MSD_DIT[f * MSD_RESULTS_SIZE],
                              DIT_value,
                              decimated_timesamples,
                              f);
#endif
...
#ifdef MSD_PLANE_DEBUG
    printf("    Performing additional boxcar: nTaps: %d; max_width_performed: "
           "%d; DIT_value/2: %d;\n",
           nTaps,
           max_width_performed,
           DIT_value);
  ...
  if(switch_to_boxcar) {
    ...
    DIT_value = last_admited_DIT_value; // each element of the input array is
                                        // sum of 'last_admited_DIT_value'
                                        // elements of initial array
    float *input_pointer,
        *output_pointer; // setup temporary pointers to input and output based
                         // on where is the last output.
    if(f % 2 == 0) {
    ...
    while(DIT_value <= max_width_performed) {
      ...
      //               in       out      :|
      nRest = PPF_L1(
          input_pointer, output_pointer, nDMs, decimated_timesamples, nTaps);
      timer.Stop();
      t_dit_time += timer.Elapsed();
...
#ifdef MSD_PLANE_DEBUG
      printf("    Performing boxcar instead of DIT: nTaps: %d; "
             "max_width_performed: %d; DIT_value: %d;\n",
             nTaps,
             max_width_performed,
             DIT_value);
      MSD_plane_profile_debug(&d_MSD_DIT[f * MSD_RESULTS_SIZE],
...
#ifdef MSD_PLANE_DEBUG
      printf("    Performing additional boxcar: nTaps: %d; "
             "max_width_performed: %d; DIT_value: %d;\n",
             nTaps,
             max_width_performed,
             last_admited_DIT_value * nTaps);
      MSD_plane_profile_debug(&d_MSD_DIT[f * MSD_RESULTS_SIZE],
                              last_admited_DIT_value * nTaps,
                              decimated_timesamples,
                              f);
  ...
  printf("    MSD of input plane: Total time: %f ms; DiT time: %f ms; MSD "
         "time: %f ms;\n",
         (*total_time),
         (*dit_time),
         (*MSD_time));
...
void MSD_Interpolate_linear(float *           mean,
                            float *           StDev,
                            float             desired_width,
                            float *           h_MSD_DIT,
                            std::vector<int> *h_MSD_DIT_widths) {
  ...
  if(position == MSD_DIT_size - 1 && width1 == (int)desired_width) {
    ...
    (*mean) = mean1 + (distance_in_mean / distance_in_width) *
                          ((float)desired_width - width1);
    (*StDev) = StDev1 + (distance_in_StDev / distance_in_width) *
                            ((float)desired_width - width1);
...
void MSD_Interpolate_square(float *           mean,
                            float *           StDev,
                            float             desired_width,
                            float *           h_MSD_DIT,
                            std::vector<int> *h_MSD_DIT_widths) {
  ...
  if(position == MSD_DIT_size - 2)
    position--;
  if(position == MSD_DIT_size - 1 &&
...
void MSD_Export_plane(const char *      filename,
                      float *           h_MSD_DIT,
                      std::vector<int> *h_MSD_DIT_widths,
  ...
  for(size_t f = 0; f < (int)h_MSD_DIT_widths->size(); f++) {
    FILEOUT << (int)h_MSD_DIT_widths->operator[](f) << " "
            << h_MSD_DIT[f * MSD_RESULTS_SIZE] << " "
            << h_MSD_DIT[f * MSD_RESULTS_SIZE + 1] << std::endl;
  }
  FILEOUT.close();
  ...
  for(size_t f = 0; f < (int)h_boxcar_widths->size(); f++) {
    if(h_boxcar_widths->operator[](f) <= max_width_performed)
      FILEOUT << (int)h_boxcar_widths->operator[](f) << " "
              << h_MSD_interpolated[f * MSD_INTER_SIZE] << " "
              << h_MSD_interpolated[f * MSD_INTER_SIZE + 1] << std::endl;
  }
  FILEOUT.close();
}

void MSD_Interpolate_values(float *           d_MSD_interpolated,
                            float *           d_MSD_DIT,
                            std::vector<int> *h_MSD_DIT_widths,
                            int               nMSDs,
                            std::vector<int> *h_boxcar_widths,
  ...
  int *d_boxcar;
  checkCudaErrors(
      cudaMalloc((void **)&d_MSD_DIT_widths, sizeof(int) * MSD_DIT_size));
  checkCudaErrors(cudaMemcpyAsync(d_MSD_DIT_widths,
                                  &h_MSD_DIT_widths->operator[](0),
                                  sizeof(int) * MSD_DIT_size,
                                  cudaMemcpyHostToDevice));
  cudaMalloc((void **)&d_boxcar, sizeof(int) * nWidths);
  checkCudaErrors(cudaMemcpyAsync(d_boxcar,
                                  &h_boxcar_widths->operator[](0),
                                  sizeof(int) * nWidths,
                                  cudaMemcpyHostToDevice));
  ...
  printf("------------------ MSD_plane_profile DEBUG - Linear interpolation "
         "------------\n");
#endif
  call_kernel_MSD_GPU_Interpolate_linear(1,
                                         nWidths,
                                         d_MSD_DIT,
                                         d_MSD_interpolated,
                                         d_MSD_DIT_widths,
                                         h_MSD_DIT_widths->size(),
                                         d_boxcar,
                                         max_width_performed);
  ...
  checkCudaErrors(cudaMemcpy(h_MSD_DIT,
                             d_MSD_DIT,
                             nMSDs * MSD_RESULTS_SIZE * sizeof(float),
                             cudaMemcpyDeviceToHost));
  checkCudaErrors(cudaMemcpy(h_MSD_interpolated,
                             d_MSD_interpolated,
                             nWidths * MSD_RESULTS_SIZE * sizeof(float),
                             cudaMemcpyDeviceToHost));
  MSD_Export_plane(filename,
                   h_MSD_DIT,
                   h_MSD_DIT_widths,
                   h_MSD_interpolated,
                   h_boxcar_widths,
...
void Get_MSD_plane_profile_memory_requirements(
    size_t *          MSD_profile_size_in_bytes,
    size_t *          MSD_DIT_profile_size_in_bytes,
    size_t *          workarea_size_in_bytes,
  ...
  // temporary storage for calculation of MSD. We have to choose the maximum
  // from all possible variants.
  ...
  for(int f = 0; f < nDecimations; f++) {
    ...
    if(conf.nBlocks_total > max_nBlocks)
      max_nBlocks = conf.nBlocks_total;
  ...
  printf("Size of DIT MSDs: %d elements = %d float = %d bytes\n",
         nDecimations,
         nDecimations * MSD_RESULTS_SIZE,
         nDecimations * MSD_RESULTS_SIZE * sizeof(float));
  printf("Max MSD blocks: %d blocks = %d float = %d bytes\n",
         max_nBlocks,
         max_nBlocks * MSD_PARTIAL_SIZE,
         max_nBlocks * MSD_PARTIAL_SIZE * sizeof(float));
#endif

  (*workarea_size_in_bytes)    = t_wsib;
  (*MSD_profile_size_in_bytes) = boxcar_widths->size() * 2 * sizeof(float);
  (*MSD_DIT_profile_size_in_bytes) =
      nDecimations * MSD_PARTIAL_SIZE * sizeof(float);
}
...
// into memory but we still plit it in two.
//		Add branch that would not split DIT=2 if there is enough memory.
void MSD_plane_profile(float *           d_MSD_interpolated,
                       float *           d_input_data,
                       float *           d_MSD_DIT_previous,
                       float *           workarea,
                       bool              high_memory,
  ...
  cudaMemset((void *)d_MSD_DIT,
             0,
             (nDecimations + 1) * MSD_RESULTS_SIZE * sizeof(float));

  MSD_of_input_plane(d_MSD_DIT,
                     &h_MSD_DIT_widths,
                     d_input_data,
                     d_MSD_DIT_previous,
                     d_sudy,
...
#ifdef MSD_PLANE_DEBUG
  printf("    Number of calculated MSD values: %d; number of interpolated MSD "
         "values: %d;\n",
         nDIT_widths,
         boxcar_widths_size);
#endif
  ...
  char filename[100];
  sprintf(filename,
          "MSD_plane_profile_i_test-t_%.2f-dm_%.2f-%.2f",
          tstart,
          dm_low,
          dm_high);
  MSD_Interpolate_values(d_MSD_interpolated,
                         d_MSD_DIT,
                         &h_MSD_DIT_widths,
                         nDIT_widths,
                         boxcar_widths,
...
void Create_boxcar_MSD(float *                d_data,
                       size_t                 nTimesamples,
                       size_t                 nDMs,
                       std::vector<MSD_Data> *boxcar_MSD,
                       std::vector<MSD_Data> *boxcar_MSD_BLN,
                       int                    max_nTaps,
  ...
  MSD_outlier_rejection(
      d_MSD, d_data, nTimesamples, nDMs, 0, OR_sigma_multiplier);
  cudaMemcpy(
      h_MSD, d_MSD, MSD_RESULTS_SIZE * sizeof(float), cudaMemcpyDeviceToHost);
  mdtemp.width = 1;
  mdtemp.mean  = h_MSD[0];
  ...
  for(size_t f = 2; f <= max_nTaps; f++) {
    if((nTimesamples - f + 1) > 0) {
      ...
      MSD_outlier_rejection(
          d_MSD, d_boxcar, nTimesamples, nDMs, nRest, OR_sigma_multiplier);
      cudaMemcpy(h_MSD,
                 d_MSD,
                 MSD_RESULTS_SIZE * sizeof(float),
                 cudaMemcpyDeviceToHost);
...
void MSD_plane_profile_boxcars(float *           d_input_data,
                               size_t            nTimesamples,
                               size_t            nDMs,
                               std::vector<int> *boxcar_widths,
                               float             OR_sigma_multiplier,
  ...
  Create_boxcar_MSD(d_input_data,
                    nTimesamples,
                    nDMs,
                    &h_boxcar_MSD,
                    &h_boxcar_MSD_OR,
  ...
  sprintf(filename,
          "MSD_boxcars_OR%f-t_%.2f-dm_%.2f-%.2f.dat",
          OR_sigma_multiplier,
          tstart,
          dm_low,
          dm_high);
  ...
  for(size_t f = 0; f < h_boxcar_MSD.size(); f++) {
    FILEOUT << (int)h_boxcar_MSD[f].width << " " << h_boxcar_MSD[f].mean << " "
            << h_boxcar_MSD[f].sd << " "
            << "3" << std::endl;
  }
  FILEOUT << std::endl;
  ...
  for(size_t f = 0; f < h_boxcar_MSD_OR.size(); f++) {
    FILEOUT << (int)h_boxcar_MSD_OR[f].width << " " << h_boxcar_MSD_OR[f].mean
            << " " << h_boxcar_MSD_OR[f].sd << " "
            << "4" << std::endl;
  }
  FILEOUT << std::endl;
  FILEOUT << std::endl;
