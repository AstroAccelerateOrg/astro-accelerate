...
Calculate_SNR_const_memory(float x, float y, ushort Taps) {
  float2 output;
...
__device__ __inline__ float2
Calculate_SNR_EACH(float x, float y, float StdDev) {
  float2 output;
...
__device__ __inline__ void Compare(float2*  active_SNR,
                                   ushort2* active_taps,
                                   float2*  passive_SNR,
                                   ushort*  passive_taps) {
  if(passive_SNR->x > active_SNR->x) {
    active_SNR->x  = passive_SNR->x;
    active_taps->x = (*passive_taps);
  }
  if(passive_SNR->y > active_SNR->y) {
    active_SNR->y  = passive_SNR->y;
    active_taps->y = (*passive_taps);
  }
}
...
__device__ __inline__ void Compare(float2*  active_SNR,
                                   ushort2* active_taps,
                                   float2*  passive_SNR,
                                   ushort2* passive_taps) {
  if(passive_SNR->x > active_SNR->x) {
    active_SNR->x  = passive_SNR->x;
    active_taps->x = passive_taps->x;
  }
  if(passive_SNR->y > active_SNR->y) {
    active_SNR->y  = passive_SNR->y;
    active_taps->y = passive_taps->y;
  }
}
//------------- device functions
//--------------------------------------------------------------------------------------

__global__ void SPDT_GPU_1st_plane(float const* __restrict__ d_input,
                                   float*  d_bv_out,
                                   float*  d_decimated,
                                   float*  d_output_SNR,
                                   ushort* d_output_taps,
                                   float2 const* __restrict__ d_MSD,
                                   int       nTimesamples,
                                   int       nBoxcars,
                                   const int dtm) {
  ...
  if((spos + 4) < nTimesamples) {
    ...
    Bw[0] = ftemp1.x;
    Bw[4] = ftemp1.y;
    Bw[1] = ftemp1.x + ftemp1.y;
    Bw[5] = ftemp1.y + ftemp2.x;
    Bw[2] = Bw[1] + ftemp2.x;
    Bw[6] = Bw[5] + ftemp2.y;
    Bw[3] = Bw[1] + ftemp2.x + ftemp2.y;
    Bw[7] = Bw[5] + ftemp2.y + ftemp3.x;
  }

  spos = blockIdx.x * (PD_NTHREADS - (nBoxcars >> 1)) + threadIdx.x;
  if(threadIdx.x < (PD_NTHREADS - (nBoxcars >> 1)) && spos < dtm)
    d_decimated[blockIdx.y * dtm + spos] = Bw[1];
  ...
  if(threadIdx.x < (PD_NTHREADS - (nBoxcars >> 1)) && spos < nTimesamples) {
    ...
    spos = blockIdx.x * (PD_NTHREADS - (nBoxcars >> 1)) + threadIdx.x;
    if(spos < dtm)
      d_bv_out[blockIdx.y * dtm + spos] = s_input[threadIdx.x].x;
  }
}

__global__ void SPDT_GPU_Nth_plane(float const* __restrict__ d_input,
                                   float*  d_bv_in,
                                   float*  d_bv_out,
                                   float*  d_decimated,
                                   float*  d_output_SNR,
  ...
  Bw[6]               = 0;
  Bw[7]               = 0;
  s_BV[threadIdx.x].x = 0;
  s_BV[threadIdx.x].y = 0;
  ...
  if(spos >= 0)
    s_input[spos].x = s_BV[spos].x + Bw[3];
...
void call_kernel_SPDT_GPU_1st_plane(const dim3&         grid_size,
                                    const dim3&         block_size,
                                    float const* const  d_input,
                                    float* const        d_bv_out,
                                    float* const        d_decimated,
                                    float* const        d_output_SNR,
                                    ushort* const       d_output_taps,
                                    float2 const* const d_MSD,
                                    const int&          nTimesamples,
                                    const int&          nBoxcars,
                                    const int&          dtm) {
  SPDT_GPU_1st_plane<<<grid_size, block_size>>>(d_input,
                                                d_bv_out,
                                                d_decimated,
                                                d_output_SNR,
                                                d_output_taps,
...
void call_kernel_SPDT_GPU_Nth_plane(const dim3&         grid_size,
                                    const dim3&         block_size,
                                    float const* const  d_input,
                                    float* const        d_bv_in,
                                    float* const        d_bv_out,
                                    ...
                                    const int&          DIT_value,
                                    const int&          dtm) {
  SPDT_GPU_Nth_plane<<<grid_size, block_size>>>(d_input,
                                                d_bv_in,
                                                d_bv_out,
                                                d_decimated,
                                                d_output_SNR,
