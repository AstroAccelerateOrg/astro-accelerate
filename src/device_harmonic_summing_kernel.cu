...
__global__ void PHS_GPU_kernel_old(float const* __restrict__ d_input,
                                   float*  d_output_SNR,
    ...
    if(blockIdx.x > 0) {
      for(int f = 1; f < nHarmonics; f++) {
        if((blockIdx.x + f * blockIdx.x) < nTimesamples) {
          pos = (blockIdx.x + f * blockIdx.x) * nSpectra +
                blockIdx.y * blockDim.x + threadIdx.x;
          HS_value = HS_value + __ldg(&d_input[pos]);
          temp_SNR = __frsqrt_rn(f + 1) * (HS_value - signal_mean * (f + 1)) /
                     (signal_sd); // assuming white noise
          if(temp_SNR > SNR) {
            SNR              = temp_SNR;
            max_SNR_harmonic = f;
          }
...
__global__ void PHS_GPU_kernel(float const* __restrict__ d_input,
                               float*  d_output_SNR,
                               ushort* d_output_harmonics,
                               float*  d_MSD,
                               int     nTimesamples,
      ...
      for(int f = 1; f < nHarmonics; f++) {
        if((blockIdx.x + f * blockIdx.x) < nTimesamples) {
          pos = (blockIdx.x + f * blockIdx.x) * nSpectra +
                blockIdx.y * blockDim.x + threadIdx.x;
          HS_value = HS_value + __ldg(&d_input[pos]);
          temp_SNR = (HS_value - __ldg(&d_MSD[f * 2])) /
                     (__ldg(&d_MSD[2 * f + 1])); // assuming white noise
          if(temp_SNR > SNR) {
            SNR              = temp_SNR;
            max_SNR_harmonic = f;
          }
...
void call_kernel_PHS_GPU_kernel_old(const dim3&        grid_size,
  ...
  PHS_GPU_kernel_old<<<grid_size, block_size>>>(d_input,
                                                d_output_SNR,
                                                d_output_harmonics,
                                                d_MSD,
                                                nTimesamples,
...
void call_kernel_PHS_GPU_kernel(const dim3&        grid_size,
                                const dim3&        block_size,
                                float const* const d_input,
                                float* const       d_output_SNR,
                                ushort* const      d_output_harmonics,
                                ...
                                const int&         nSpectra,
                                const int&         nHarmonics) {
  PHS_GPU_kernel<<<grid_size, block_size>>>(d_input,
                                            d_output_SNR,
                                            d_output_harmonics,
                                            d_MSD,
                                            nTimesamples,
