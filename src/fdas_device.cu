...
do_FFT_no_reorder(float2* s_input, int N, int bits) { // in-place
  float2 A_DFT_value, B_DFT_value, ftemp2, ftemp;
  float2 WB;

  int r, Bj, Bk, PoTm1, A_read_index, B_read_index, A_write_index,
      B_write_index, Nhalf;
...
static __device__ __inline__ void
do_FFT_CT_DIF_2elem_no_reorder(float2* s_input) {
...
static __device__ __inline__ void
do_FFT_CT_DIF_4elem_no_reorder(float2* s_input) {
...
static __device__ __inline__ void
do_IFFT_no_reorder(float2* s_input, int N, int bits) { // in-place
  float2 A_DFT_value, B_DFT_value;
  float2 W;
  float2 ftemp, ftemp2;
  ...
  if(threadIdx.x < Nhalf) {
    ...
    A_DFT_value.x =
        parity * A_DFT_value.x + __shfl(A_DFT_value.x, local_id + parity);
    A_DFT_value.y =
        parity * A_DFT_value.y + __shfl(A_DFT_value.y, local_id + parity);

    B_DFT_value.x =
        parity * B_DFT_value.x + __shfl(B_DFT_value.x, local_id + parity);
    B_DFT_value.y =
        parity * B_DFT_value.y + __shfl(B_DFT_value.y, local_id + parity);
    ...
    for(q = 2; q < 6; q++) {
      ...
      A_DFT_value.x =
          __shfl(A_DFT_value.x, A_read_index) + W.x * ftemp2.x - W.y * ftemp2.y;
      A_DFT_value.y =
          __shfl(A_DFT_value.y, A_read_index) + W.x * ftemp2.y + W.y * ftemp2.x;

      ftemp.x = __shfl(B_DFT_value.x, B_read_index);
      ftemp.y = __shfl(B_DFT_value.y, B_read_index);
      B_DFT_value.x =
          __shfl(B_DFT_value.x, A_read_index) + W.x * ftemp.x - W.y * ftemp.y;
      B_DFT_value.y =
          __shfl(B_DFT_value.y, A_read_index) + W.x * ftemp.y + W.y * ftemp.x;
...
static __device__ __inline__ void
do_IFFT_mk11_2elem_2vertical_no_reorder(float2* s_input_1, float2* s_input_2) {
...
static __device__ __inline__ void
do_IFFT_mk11_2elem_no_reorder(float2* s_input) {
  float2 A_DFT_value, B_DFT_value;
  float2 W;
  float2 ftemp, ftemp2;
...
static __device__ __inline__ void
do_IFFT_mk11_4elem_no_reorder(float2* s_input) {
  float2 A_DFT_value, B_DFT_value, C_DFT_value, D_DFT_value;
  float2 W;
  float2 Aftemp, Bftemp, Cftemp, Dftemp;
...
static __device__ __inline__ void
do_IFFT_mk11_4elem_2vertical_no_reorder(float2* s_input1, float2* s_input2) {
...
__global__ void cuda_overlap_copy(float2* d_ext_data,
                                  float2* d_cpx_signal,
                                  int     sigblock,
                                  int     sig_rfftlen,
                                  int     sig_tot_convlen,
  ...
  for(int i = 1; i < total_blocks; i++) { // copy overlapped blocks
    d_ext_data[(i * KERNLEN + tid)] =
        d_cpx_signal[i * sigblock - kern_offset + tid];
  }
}

__global__ void cuda_overlap_copy_smallblk(float2* d_ext_data,
                                           float2* d_cpx_signal,
                                           int     sigblock,
                                           int     sig_rfftlen,
                                           int     sig_tot_convlen,
...
__global__ void cuda_convolve_reg_1d_halftemps(float2* d_kernel,
                                               float2* d_signal,
                                               float2* d_ffdot_plane,
                                               int     sig_tot_convlen,
                                               float   scale) {
...
#pragma unroll
    for(int i = 0; i < ZMAX / 2; i++) {
      // upper half
      d_ffdot_plane[i * sig_tot_convlen + j + tidx].x =
          (local_data.x * tempkern[i].x - local_data.y * tempkern[i].y) * scale;
      d_ffdot_plane[i * sig_tot_convlen + j + tidx].y =
          (local_data.x * tempkern[i].y + local_data.y * tempkern[i].x) * scale;
      // complex conjugate filter lower half
      d_ffdot_plane[(ZMAX - i) * sig_tot_convlen + j + tidx].x =
          (local_data.x * tempkern[i].x + local_data.y * tempkern[i].y) * scale;
      d_ffdot_plane[(ZMAX - i) * sig_tot_convlen + j + tidx].y =
          (-local_data.x * tempkern[i].y + local_data.y * tempkern[i].x) *
          scale;
    }

    // do z=0
    float2 tempz = __ldg(&d_kernel[(ZMAX / 2) * KERNLEN + tidx]);
    d_ffdot_plane[(ZMAX / 2) * sig_tot_convlen + j + tidx].x =
        (local_data.x * tempz.x - local_data.y * tempz.y) * scale;
    d_ffdot_plane[(ZMAX / 2) * sig_tot_convlen + j + tidx].y =
        (local_data.x * tempz.y + local_data.y * tempz.x) * scale;
  }
}

__global__ void cuda_ffdotpow_concat_2d(float2* d_ffdot_plane_cpx,
                                        float*  d_ffdot_plane,
                                        int     sigblock,
                                        int     kern_offset,
                                        int     total_blocks,
                                        int     sig_tot_convlen,
                                        int     sig_totlen)
/* Copies useful points from convolved complex f-fdot array (discarding
   contaminated ends) */
/* calculates complex signal power and places result in float f-fdot array */
{
  ...
  for(int i = 0; i < total_blocks; ++i) {
    if(tidx < sigblock) {
      float2 local_data =
          __ldg(&d_ffdot_plane_cpx[(tidy * sig_tot_convlen) + i * KERNLEN +
                                   tidx + kern_offset]);
      d_ffdot_plane[(tidy * sig_totlen) + i * sigblock + tidx] =
          (local_data.x * local_data.x + local_data.y * local_data.y);
    }
  }
}

__global__ void cuda_ffdotpow_concat_2d_inbin(float2* d_ffdot_plane_cpx,
                                              float*  d_ffdot_plane,
                                              int     sigblock,
                                              int     kern_offset,
                                              int     total_blocks,
...
/* Copies useful points from convolved complex f-fdot array (discarding
   contaminated ends) */
/* calculates complex signal power and places result in float f-fdot array */
/* This one performs interbinning on the complex signal before power
   calculations */
{
  int tx   = threadIdx.x;
  int bx   = blockIdx.x;
  ...
  for(int i = 0; i < total_blocks; ++i) {
    ...
    if(tidx < sigblock) {
      ...
      // onto shared memory
      local_data[tx] =
          __ldg(&d_ffdot_plane_cpx[(by * sig_tot_convlen) + i * KERNLEN + tidx +
                                   kern_offset]);
      __syncthreads();
      ...
      if(tx == PTBSIZEX - 1)
        local_data[tx + 1] =
            __ldg(&d_ffdot_plane_cpx[(by * sig_tot_convlen) + i * KERNLEN +
                                     tidx + kern_offset + 1]);
      __syncthreads();
      ...
      if(tidx == sigblock - 1) {
        //	 if(tx==PTBSIZEX-1)
        local_data[tx + 1] =
            __ldg(&d_ffdot_plane_cpx[(by * sig_tot_convlen) +
                                     (i + 1) * KERNLEN + kern_offset]);
      }
      __syncthreads();

      // spread signal in interleaving bins within each block, and write power
      // to each even bin
      local_data_inbin[2 * tx] = local_data[tx].x * local_data[tx].x +
                                 local_data[tx].y * local_data[tx].y;
      __syncthreads();

      // fill in the empty bins:
      // P_k+1/2 = |F_k+1/2|^2 =~|pi/4 * (F_k - F_k-1)|^2
      // (see Handbook of Pulsar astronomy p.135)
      local_data_inbin[2 * tx + 1] =
          0.616850275f * ((local_data[tx].x - local_data[tx + 1].x) *
                              (local_data[tx].x - local_data[tx + 1].x) +
                          (local_data[tx].y - local_data[tx + 1].y) *
                              (local_data[tx].y - local_data[tx + 1].y));
      __syncthreads();

      // write data back to global memory contiguously
      unsigned int inbin_idx =
          (unsigned int)(2 * by * sig_totlen + i * 2 * sigblock +
                         2 * bx * blockDim.x) +
          tx;
...
__global__ void cuda_ffdotpow_concat_2d_ndm_inbin(float2* d_ffdot_plane_cpx,
                                                  float*  d_ffdot_plane,
                                                  int     kernlen,
                                                  int     siglen,
                                                  int     nkern,
   ...
   calculations */
{
  ...
  for(int dm = 0; dm < ndm; dm++) {
    ...
    for(int i = 0; i < total_blocks; i++) {
      //       __syncthreads();
      if(tidx < siglen) {
        // first load complex f-fdot blocks between overlap offsets onto shared
        // memory
        local_data[tx] =
            __ldg(&d_ffdot_plane_cpx[dmidx_cpx + (by * sig_tot_convlen) +
                                     i * kernlen + tidx + kern_offset]);
        __syncthreads();
        // load one extra point in the x direction
        if(tx == PTBSIZEX - 1)
          local_data[tx + 1] =
              __ldg(&d_ffdot_plane_cpx[dmidx_cpx + (by * sig_tot_convlen) +
                                       i * kernlen + tidx + kern_offset + 1]);
        __syncthreads();
        // pick first points from the next segment, in i+1
        if(tidx == siglen - 1) {
          // if(tx==PTBSIZEX-1)
        ...
        local_data_inbin[2 * tx] = local_data[tx].x * local_data[tx].x +
                                   local_data[tx].y * local_data[tx].y;
        __syncthreads();
        // fill in the empty bins: P_k+1/2 = |F_k+1/2|^2 =~|pi/4 * (F_k -
        // F_k-1)|^2 (see Handbook of Pulsar astronomy p.135)
        ...
        d_ffdot_plane[2 * dmidx_f + 2 * (by * sig_totlen) + i * 2 * siglen +
                      2 * bx * blockDim.x + tx + PTBSIZEX] =
            local_data_inbin[tx + PTBSIZEX];
        //	}
        // __syncthreads();
      }
    }
...
__global__ void cuda_convolve_customfft_wes_no_reorder02(float2* d_kernel,
                                                         float2* d_signal,
                                                         float*  d_ffdot_pw,
                                                         int     sigblock,
                                                         int   sig_tot_convlen,
                                                         ...
                                                         int   offset,
                                                         float scale)
/* convolution kernel using Karel Adamek's custom FFT, deployed here with
   modifications by Wes Armour. It performs the forward FFT and then loops
   through filters.(1-d blocks) It also uses half the templates and computes the
   rest using the complex conjugate. The modifications are optimizing speed by
   computing FFTs on two templates with one synchronization point. In this
   version we apply Karel's kernel with removed de-shuffling of the data during
   the FFT.*/
{
  int    tx = threadIdx.x;
  int    bx = blockIdx.x;
...
#pragma unroll
  for(int i = 0; i < ZMAX / 2; i++) {
    ...
    if(threadIdx.x < KERNLEN / 2) {
      ...
      sA_DFT_value.x =
          parity * sA_DFT_value.x + __shfl(sA_DFT_value.x, local_id + parity);
      sA_DFT_value.y =
          parity * sA_DFT_value.y + __shfl(sA_DFT_value.y, local_id + parity);
      sB_DFT_value.x =
          parity * sB_DFT_value.x + __shfl(sB_DFT_value.x, local_id + parity);
      sB_DFT_value.y =
          parity * sB_DFT_value.y + __shfl(sB_DFT_value.y, local_id + parity);
      ...
      for(q = 2; q < 6; q++) {
        ...
        //	W=Get_W_value_inverse(PoT*2,m_param);
        W.x = cosf(2.0f * 3.141592654f *
                   fdividef((float)m_param, (float)(PoT * 2)));
        W.y = sinf(2.0f * 3.141592654f *
                   fdividef((float)m_param, (float)(PoT * 2)));
        ...
        // 1st template
        A_DFT_value.x = __shfl(A_DFT_value.x, A_read_index) + W.x * ftemp2.x -
                        W.y * ftemp2.y;
        A_DFT_value.y = __shfl(A_DFT_value.y, A_read_index) + W.x * ftemp2.y +
                        W.y * ftemp2.x;
        // 2nd template
        sA_DFT_value.x = __shfl(sA_DFT_value.x, A_read_index) + W.x * stemp2.x -
                         W.y * stemp2.y;
        sA_DFT_value.y = __shfl(sA_DFT_value.y, A_read_index) + W.x * stemp2.y +
                         W.y * stemp2.x;
        ...
        B_DFT_value.y =
            __shfl(B_DFT_value.y, A_read_index) + W.x * ftemp.y + W.y * ftemp.x;
        // 2nd template
        sB_DFT_value.x = __shfl(sB_DFT_value.x, A_read_index) + W.x * stemp.x -
                         W.y * stemp.y;
        sB_DFT_value.y = __shfl(sB_DFT_value.y, A_read_index) + W.x * stemp.y +
                         W.y * stemp.x;
    ...
    if(tx >= offset && tx < sigblock + offset) {
      ...
      d_ffdot_pw[(ZMAX - i) * sig_totlen + index - offset] =
          pwcalc(s_input_trans[tx]);
    }
  }
...
__global__ void
cuda_convolve_customfft_wes_no_reorder02_inbin(float2* d_kernel,
                                               float2* d_signal,
                                               float*  d_ffdot_pw,
                                               int     sigblock,
                                               int     sig_tot_convlen,
                                               ...
                                               int     offset,
                                               float   scale,
                                               float2* ip_edge_points)
/* convolution kernel using Karel Adamek's custom FFT, deployed here with
   modifications by Wes Armour. It performs the forward FFT and then loops
   through filters.(1-d blocks) It also uses half the templates and computes the
   rest using the complex conjugate. The modifications are optimizing speed by
   computing FFTs on two templates with one synchronization point. Karel's
   kernel with removed de-shuffling of the data during the FFT.
   In this version we apply interbinning (fourier amplitudes interpolation in
   two bins)*/
{
  int    tx = threadIdx.x;
  int    bx = blockIdx.x;
...
#pragma unroll
  for(int i = 0; i < ZMAX / 2; i++) {
    ...
    if(tx == offset) {
      ...
      if(bx < gridDim.x - 1)
        s_input_trans[tx + sigblock] =
            __ldg(&ip_edge_points[bx + gridDim.x + 1]);
    }
    __syncthreads();
    ...
    if(tx < sigblock) {
      ...
      s_output_ip[2 * tx + 1] =
          0.616850275f *
          ((s_input[tx + offset].x - s_input[tx + 1 + offset].x) *
               (s_input[tx + offset].x - s_input[tx + 1 + offset].x) +
           (s_input[tx + offset].y - s_input[tx + 1 + offset].y) *
               (s_input[tx + offset].y - s_input[tx + offset + 1].y));
      s_output_ip_trans[2 * tx + 1] =
          0.616850275 *
          ((s_input_trans[tx + offset].x - s_input_trans[tx + 1 + offset].x) *
               (s_input_trans[tx + offset].x -
                s_input_trans[tx + 1 + offset].x) +
           (s_input_trans[tx + offset].y - s_input_trans[tx + 1 + offset].y) *
               (s_input_trans[tx + offset].y -
                s_input_trans[tx + 1 + offset].y));
    }
    __syncthreads();
    ...
    if(tx < sigblock) {
      ...
      d_ffdot_pw[(i * sig_inbin_len + index + sigblock)] =
          s_output_ip[tx + sigblock];

      // 2nd tempalte
      d_ffdot_pw[(unsigned int)((ZMAX - i) * sig_inbin_len + index)] =
          s_output_ip_trans[tx];
      d_ffdot_pw[(unsigned int)((ZMAX - i) * sig_inbin_len + index +
                                sigblock)] = s_output_ip_trans[tx + sigblock];
    }
  }
  ...
  if(tx < sigblock) {
    ...
    d_ffdot_pw[((ZMAX / 2) * sig_inbin_len + index + sigblock)] =
        s_output_ip[tx + sigblock];
  }
  //--------END
}
...
__global__ void
GPU_CONV_kFFT_mk11_2elem_2v(float2 const* __restrict__ d_input_signal,
                            float* d_output_plane_reduced,
                            float2 const* __restrict__ d_templates,
  ...
  s_input_1[threadIdx.x] =
      __ldg(&d_input_signal[blockIdx.x * KERNLEN + threadIdx.x]);
  s_input_1[threadIdx.x + (KERNLEN >> 1)] = __ldg(
      &d_input_signal[blockIdx.x * KERNLEN + threadIdx.x + (KERNLEN >> 1)]);
  ...
  for(t = 0; t < ZMAX / 2; t++) {
    ...
    // Convolution
    s_input_1[threadIdx.x].x =
        (r_templates_1[0].x * signal[0].x - r_templates_1[0].y * signal[0].y) *
        scale;
    s_input_1[threadIdx.x].y =
        (r_templates_1[0].x * signal[0].y + r_templates_1[0].y * signal[0].x) *
        scale;

    s_input_2[threadIdx.x].x =
        (r_templates_2[0].x * signal[0].x + r_templates_2[0].y * signal[0].y) *
        scale;
    s_input_2[threadIdx.x].y =
        (r_templates_2[0].x * signal[0].y - r_templates_2[0].y * signal[0].x) *
        scale;

    s_input_1[threadIdx.x + (KERNLEN >> 1)].x =
        (r_templates_1[1].x * signal[1].x - r_templates_1[1].y * signal[1].y) *
        scale;
    s_input_1[threadIdx.x + (KERNLEN >> 1)].y =
        (r_templates_1[1].x * signal[1].y + r_templates_1[1].y * signal[1].x) *
        scale;

    s_input_2[threadIdx.x + (KERNLEN >> 1)].x =
        (r_templates_2[1].x * signal[1].x + r_templates_2[1].y * signal[1].y) *
        scale;
    s_input_2[threadIdx.x + (KERNLEN >> 1)].y =
        (r_templates_2[1].x * signal[1].y - r_templates_2[1].y * signal[1].x) *
        scale;
    ...
    // Saving data
    pos = t * useful_part_size * nConvolutions + blockIdx.x * useful_part_size +
          threadIdx.x;
    if(threadIdx.x >= offset && threadIdx.x < (useful_part_size + offset)) {
      d_output_plane_reduced[pos - offset] = pwcalc(s_input_1[threadIdx.x]);
    }
    if((threadIdx.x + (KERNLEN >> 1)) >= offset &&
       (threadIdx.x + (KERNLEN >> 1)) < (useful_part_size + offset)) {
      d_output_plane_reduced[pos + (KERNLEN >> 1) - offset] =
          pwcalc(s_input_1[threadIdx.x + (KERNLEN >> 1)]);
    }
    pos = (ZMAX - t) * useful_part_size * nConvolutions +
          blockIdx.x * useful_part_size + threadIdx.x;
    if(threadIdx.x >= offset && threadIdx.x < (useful_part_size + offset)) {
    ...
    if((threadIdx.x + (KERNLEN >> 1)) >= offset &&
       (threadIdx.x + (KERNLEN >> 1)) < (useful_part_size + offset)) {
      d_output_plane_reduced[pos + (KERNLEN >> 1) - offset] =
          pwcalc(s_input_2[threadIdx.x + (KERNLEN >> 1)]);
    }
  }
  ...
  pos = (ZMAX / 2) * useful_part_size * nConvolutions +
        blockIdx.x * useful_part_size + threadIdx.x;
  if(threadIdx.x >= offset && threadIdx.x < (useful_part_size + offset)) {
    d_output_plane_reduced[pos - offset] = pwcalc(s_input_1[threadIdx.x]);
  }
...
__global__ void
GPU_CONV_kFFT_mk11_4elem_2v(float2 const* __restrict__ d_input_signal,
                            float* d_output_plane_reduced,
                            float2 const* __restrict__ d_templates,
                            int   useful_part_size,
                            int   offset,
  ...
  s_input_1[threadIdx.x] =
      __ldg(&d_input_signal[blockIdx.x * KERNLEN + threadIdx.x]);
  s_input_1[threadIdx.x + (KERNLEN >> 2)] = __ldg(
      &d_input_signal[blockIdx.x * KERNLEN + threadIdx.x + (KERNLEN >> 2)]);
  s_input_1[threadIdx.x + (KERNLEN >> 1)] = __ldg(
      &d_input_signal[blockIdx.x * KERNLEN + threadIdx.x + (KERNLEN >> 1)]);
  s_input_1[threadIdx.x + 3 * (KERNLEN >> 2)] = __ldg(
      &d_input_signal[blockIdx.x * KERNLEN + threadIdx.x + 3 * (KERNLEN >> 2)]);
  ...
  for(t = 0; t < ZMAX / 2; t++) {
    ...
    if((threadIdx.x + (KERNLEN >> 2)) >= offset &&
       (threadIdx.x + (KERNLEN >> 2)) < (useful_part_size + offset)) {
      d_output_plane_reduced[pos + (KERNLEN >> 2) - offset] =
          pwcalc(s_input_1[threadIdx.x + (KERNLEN >> 2)]);
    }
    if((threadIdx.x + (KERNLEN >> 1)) >= offset &&
       (threadIdx.x + (KERNLEN >> 1)) < (useful_part_size + offset)) {
      d_output_plane_reduced[pos + (KERNLEN >> 1) - offset] =
    ...
    if((threadIdx.x + 3 * (KERNLEN >> 2)) >= offset &&
       (threadIdx.x + 3 * (KERNLEN >> 2)) < (useful_part_size + offset)) {
      d_output_plane_reduced[pos + 3 * (KERNLEN >> 2) - offset] =
          pwcalc(s_input_1[threadIdx.x + 3 * (KERNLEN >> 2)]);
    }
    ...
    if((threadIdx.x + (KERNLEN >> 2)) >= offset &&
       (threadIdx.x + (KERNLEN >> 2)) < (useful_part_size + offset)) {
      d_output_plane_reduced[pos + (KERNLEN >> 2) - offset] =
          pwcalc(s_input_2[threadIdx.x + (KERNLEN >> 2)]);
    }
    if((threadIdx.x + (KERNLEN >> 1)) >= offset &&
       (threadIdx.x + (KERNLEN >> 1)) < (useful_part_size + offset)) {
      d_output_plane_reduced[pos + (KERNLEN >> 1) - offset] =
    ...
    if((threadIdx.x + 3 * (KERNLEN >> 2)) >= offset &&
       (threadIdx.x + 3 * (KERNLEN >> 2)) < (useful_part_size + offset)) {
      d_output_plane_reduced[pos + 3 * (KERNLEN >> 2) - offset] =
          pwcalc(s_input_2[threadIdx.x + 3 * (KERNLEN >> 2)]);
    }
  }
  ...
  s_input_1[threadIdx.x].y =
      (r_templates_1[0].x * signal[0].y + r_templates_1[0].y * signal[0].x) *
      scale;
  s_input_1[threadIdx.x + (KERNLEN >> 2)].x =
      (r_templates_1[1].x * signal[1].x - r_templates_1[1].y * signal[1].y) *
      scale;
  s_input_1[threadIdx.x + (KERNLEN >> 2)].y =
      (r_templates_1[1].x * signal[1].y + r_templates_1[1].y * signal[1].x) *
      scale;
  s_input_1[threadIdx.x + (KERNLEN >> 1)].x =
      (r_templates_1[2].x * signal[2].x - r_templates_1[2].y * signal[2].y) *
      scale;
  s_input_1[threadIdx.x + (KERNLEN >> 1)].y =
      (r_templates_1[2].x * signal[2].y + r_templates_1[2].y * signal[2].x) *
      scale;
  s_input_1[threadIdx.x + 3 * (KERNLEN >> 2)].x =
      (r_templates_1[3].x * signal[3].x - r_templates_1[3].y * signal[3].y) *
      scale;
  s_input_1[threadIdx.x + 3 * (KERNLEN >> 2)].y =
      (r_templates_1[3].x * signal[3].y + r_templates_1[3].y * signal[3].x) *
      scale;
