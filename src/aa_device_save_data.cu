...
namespace astroaccelerate {
  ...
  /** \brief Copy data and set up the GPU constants/variables. */
  void save_data_offset_stream(float *device_pointer, size_t device_offset, float *host_pointer, size_t host_offset, size_t size, cudaStream_t stream) {
	cudaMemcpyAsync(host_pointer + host_offset, device_pointer + device_offset, size, cudaMemcpyDeviceToHost, stream);
  }
