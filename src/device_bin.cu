...
void bin_gpu(unsigned short* d_input, float* d_output, int nchans, int nsamp) {
  ...
  // printf("\ndivisions_in_t:%d\tdivisions_in_f:%d",divisions_in_t,
  // divisions_in_f);
  ...
  // start_t = clock();
  // REPLACE WITH CALL TO C-WRAPPER FUNCTION
  ...
  // REPLACE WITH CALL TO C-WRAPPER FUNCTION
  call_kernel_swap(swap_num_blocks,
                   swap_threads_per_block,
                   d_input,
                   d_output,
                   nchans,
                   nsamp);
  ...
  // printf("\nGops based on %.2f ops per channel per tsamp:
  // %f",14.0,((15.0*(divisions_in_t*divisions_in_f*num_blocks_t*num_blocks_f))/(time))/1000000000.0);
  // printf("\nBN Device memory bandwidth in GB/s: %f",
  // (2*(sizeof(float)+sizeof(unsigned
  // short))*(divisions_in_t*divisions_in_f*num_blocks_t*num_blocks_f))/(time)/1000000000.0);
